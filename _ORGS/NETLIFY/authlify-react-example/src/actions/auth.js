...
export function logout() {
  return (dispatch, getState) => {
    ...
    return user.logout().then(() => dispatch(logoutUser()));
  };
}
...
export function signup(email, password, data) {
  return (dispatch, getState) => {
    return authlify
      ...
      .then((success) => dispatch({ type: "SIGNUP_SUCCESS" }));
  };
}
...
export function sendRecovery(email) {
  return (dispatch, getState) => {
    return authlify
      ...
      .then((success) => dispatch({ type: "RECOVER_REQUEST_SUCCESS" }));
  };
}

export function recover(code) {
  return (dispatch, getState) => {
    return authlify.recover(code).then((user) => dispatch(loginSuccess(user)));
  };
}

export function confirm(code) {
  return (dispatch, getState) => {
    return authlify.confirm(code).then((user) => dispatch(loginSuccess(user)));
  };
}
