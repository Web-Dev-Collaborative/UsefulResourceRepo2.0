$(document).ready(function () {
  ...
  var stripTags = function (html) {
    return html.replace(/(<([^>]+)>)/gi, "").split(":")[0];
  };
  ...
  var setupAudio = function (word) {
    ...
    apiUrl =
      ...
      word +
      "/audio?useCanonical=true&limit=3&api_key=" +
      apiKey;
    $.ajax({
      url: apiUrl,
  ...
  var successCallback = function (data) {
    ...
    $(".page-title").html(
      wordData[0].content + " (" + stripTags(wordData[1].content) + ")"
    );
    ...
    if (!navigator.onLine) {
      ...
      $(".player").attr(
        "title",
        "Pronunciation requires an internet connection."
      );
    }
  };
