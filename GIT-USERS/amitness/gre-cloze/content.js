...
function node_needs_to_be_checked(node) {
  ...
  let node_parent_contains_highlighter = node_has_parent
    ? node.parentElement.classList.contains("highlighter")
    : false;
  ...
  return (
    node_has_parent &&
    !node_parent_contains_highlighter &&
    node_text_not_all_empty
  );
}
...
function highlight_text_node_with_found_words(node, found_words) {
  ...
  found_words.forEach(function (word) {
    node_text_value = node_text_value.replace(
      word,
      `<span class="highlighter highlight-on">${word}</span><a href="https://www.vocabulary.com/dictionary/${word}" target="_blank">🔑</a>`
    );
...
function run() {
  try {
    chrome.storage.sync.get(["wordlist", "switch"], function (result) {
      ...
      textNodesUnder(document.getElementsByTagName("body")[0]).forEach(
        function (node) {
          //console.log(node)

          if (
            node_needs_to_be_checked(node) &&
            (found_words = find_key_words_in_node(node, wordlist))
          ) {
            //console.log("node passed check")
            updated_text_node_value = highlight_text_node_with_found_words(
              node,
              found_words
            );
