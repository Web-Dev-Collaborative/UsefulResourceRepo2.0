...
neuralNetworkCalculationViz.prototype.drawGraph = function (inputs) {
  ...
  var layers = inputs.layers,
    ...
    layerXScale = util.getLayerXScale(
      numberOfLayers,
  ...
  for (var j = 0; j < numberOfLayers; j++) {
    layers[j].nodeRenderingDetails = layers[j].renderer.getNodeRenderingDetails(
      layers[j],
      this.properties
    );
  }
...
neuralNetworkCalculationViz.prototype.drawGraphOld = function () {
  ...
  this.getYCoordinateOfNodeInInputLayer = util.getNodeYCoordinate(
  ...
  this.getYCoordinateOfNodeInOutputLayer = util.getNodeYCoordinate(
