var multiVariableNeuralNetworkTrainer = function (
  numberOfInputNodes,
  svg_el,
  table_el,
  dataPoints,
  ...
  var trainerSelf = this,
    arr = [];

  for (var c = 0; c < this.weights.length; c++) {
    $(this.table_el + " #weight" + c + "Slider").on(
      "input change",
      { key_id: c },
      function (e) {
...
multiVariableNeuralNetworkTrainer.prototype.initializeErrorGraph = function () {
  ...
  this.errorHolder = d3
    .select(this.svg_el) // select the 'div' element to append the svg
  ...
  this.errorChartWidth =
    +this.errorHolder.attr("width") - this.margin.left - this.margin.right;
  this.errorChartHeight =
    +this.errorHolder.attr("height") - this.margin.top - this.margin.bottom;
...
multiVariableNeuralNetworkTrainer.prototype.updateParameter = function (
  key,
  key_id,
  value,
  updateUI
  ...
  if (key == "weight") this.weights[key_id] = value;
  else if (key == "bias") this.bias = value;
  ...
  if (updateUI) this.updateUI(mean_delta_sum);

  return mean_delta_sum;
};

multiVariableNeuralNetworkTrainer.prototype.updateUI = function (
  mean_delta_sum
) {
...
multiVariableNeuralNetworkTrainer.prototype.batchAddErrorPoint = function (
  valuesArray
) {
...
multiVariableNeuralNetworkTrainer.prototype.rescaleErrorGraph = function () {
  ...
  this.errorG
    .select(this.svg_el + " .axis--y")
    .call(d3.axisLeft(this.error_y).ticks(5));

  this.errorG.selectAll(this.svg_el + " .axis--y .tick text").attr(
    "fill",
    function (d) {
...
multiVariableNeuralNetworkTrainer.prototype.calculatePrediction = function (x) {
  var result = 0;
  for (var c = 0; c < x.length; c++) result = result + x[c] * this.weights[c];

  return result + this.bias;
};

// Draw the neural network figure. Currently only works for two inputs.
multiVariableNeuralNetworkTrainer.prototype.initializeNeuralNetworkGraph =
  function () {
    ...
    var weight1GroupX =
        ...
        10,
      weight1GroupY =
        (weight1GroupX - this.neuralNetworkMargin.left) * 0.6 -
        this.neuralNetworkMargin.top;
    this.weightG = this.neuralNetworkG
      .append("g")
      .attr(
        "transform",
        "translate(" + weight1GroupX + "," + weight1GroupY + ")"
    ...
    this.inputNode = this.neuralNetworkG
      ...
      .attr(
        "cy",
        this.neuralNetworkHeight + this.neuralNetworkMargin.top - nodeRadius
      );
    ...
    this.weightG = this.neuralNetworkG
      ...
      .attr(
        "transform",
        "translate(" + weight2GroupX + "," + weight2GroupY + ")"
      ); //this.neuralNetworkMargin.top + this.neuralNetworkHeight / 2
    this.weightNode = this.weightG
      .append("ellipse")
      .attr("class", "weightNode")
    ...
    this.biasG = this.neuralNetworkG
      .append("g")
      .attr("transform", "translate(" + biasNodeX + "," + biasNodeY + ")");
...
multiVariableNeuralNetworkTrainer.prototype.updateNeuralNetworkGraph =
  function () {
    for (var c = 0; c < this.weights.length; c++) {
      d3.select(this.neuralNetworkGraphEl + " #weight" + c + "Value").text(
        this.weights[c].toFixed(3)
      );
    }
...
multiVariableNeuralNetworkTrainer.prototype.gradientDescentStep = function (
  steps
) {
  ...
  for (var c = 0; c < steps; c++) {
    ...
    // Fill with zeros
    sumsForWeights = Array.apply(null, Array(this.numberOfInputNodes)).map(
    ...
    for (var i = 0; i < this.dataPoints.length; i++) {
      for (var j = 0; j < this.numberOfInputNodes; j++)
        sumsForWeights[j] =
          sumsForWeights[j] +
          (this.prediction[i] - this.labels[i]) * this.dataPoints[i][j];
    ...
    if (c == steps - 1) {
      ...
      d3.range(this.numberOfInputNodes).map(
        function (id) {
...
var labels = [
  399.9, 329.9, 369.0, 232.0, 539.9, 299.9, 314.9, 198.999, 212.0, 242.5,
];
...
var weights = [0, 0],
  ...
  weightRanges,
  biasRange;
var trainer3 = new multiVariableNeuralNetworkTrainer(
  2,
  "#training-two-chart",
  "#training-two-table",
  dataPoints,
