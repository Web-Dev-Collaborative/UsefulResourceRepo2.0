...
var reluGraph = function (
  ...
  this.updateReluValue(
    this.containerElement,
    this.reluInputElement,
    this.xDefault
  );
};
...
reluGraph.prototype.initializeGraph = function () {
  var obj = this;
  this.graphHolder = d3
    .select(this.containerElement) // select the 'body' element
  ...
  function make_y_gridlines() {
    return d3.axisLeft(obj.y).ticks(1);
  }
...
reluGraph.prototype.updateGraph = function () {
  ...
  d3.select(".relu-line").attr("d", line);
};
...
reluGraph.prototype.attachEventsToSlider = function () {
  ...
  $(this.sliderElement).on("input change", function () {
    self.updateReluValue(
      self.containerElement,
      self.reluInputElement,
      this.value
    );
  });
...
reluGraph.prototype.updateReluValue = function (
  containerElement,
  sliderElement,
  inputValue
) {
  // Update Text values
  d3.selectAll(this.reluInputElement).text(inputValue);
  d3.select(this.reluFormulaInputElement).text(
    parseFloat(inputValue).toFixed(2)
  );
  // Update formula result
  d3.select(this.reluResult).text(relu(inputValue));
  d3.select(this.reluGraphResult).text(relu(inputValue));

  // Update graph
  d3.select(this.containerElement + " .value-point").attr(
    "transform",
    "translate(" + this.x(inputValue) + "," + this.y(relu(inputValue)) + ")"
  );

  // Update graph dot label
  d3.select(this.containerElement + " .relu-value-text").text(relu(inputValue));
};
