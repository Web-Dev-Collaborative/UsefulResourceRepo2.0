...
var NN_trainer = function (
  ...
  x2,
  y2,
  gradientDescentButton,
  gradientDescent10Button,
  gradientDescent100Button,
  ...
  if (normalize) this.normalizeFeatures(areas, prices);

  this.initializeGraph();

  if (error_chart_el != "") this.initializeErrorGraph();
  if (this.heatmap_el != "") this.initializeHeatmap();
  if (this.neuralNetworkGraphEl != "") this.initializeNeuralNetworkGraph();
  ...
  $(this.table_el + " #weightSlider").on("mouseup touchend", function () {
    ga(
      "send",
      "event",
      trainer_self.analyticsCategory,
      "Interacted with",
      "Weight slider"
    );
    _paq.push([
...
NN_trainer.prototype.normalizeFeatures = function (areas, prices) {
  ...
  this.areas_normalized = normalizeFeaturesArray(
    areas,
    this.area_mean,
    this.area_std
  );
  ...
  this.prices_normalized = normalizeFeaturesArray(
    prices,
    this.prices_mean,
    this.prices_std
  );
...
NN_trainer.prototype.initializeGraph = function () {
  this.holder = d3
    .select(this.svg_el) // select the 'body' element
  ...
  (this.margin = { top: 20, right: 20, bottom: 50, left: 50 }),
    (this.width =
      +this.holder.attr("width") - this.margin.left - this.margin.right),
    (this.height =
      +this.holder.attr("height") - this.margin.top - this.margin.bottom),
    (this.g = this.holder
      .append("g")
      .attr(
        "transform",
  ...
  this.dataPointDots = this.g
    .selectAll(this.svg_el + " .dot")
...
NN_trainer.prototype.initializeErrorGraph = function () {
  ...
  this.errorHolder = d3
    .select(this.error_chart_el) // select the 'body' element
    .append("svg") // append an SVG element to the body
  ...
  this.errorChartWidth =
    +this.errorHolder.attr("width") -
    this.miniErrorChartMargin.left -
    this.miniErrorChartMargin.right;
  this.errorChartHeight =
    +this.errorHolder.attr("height") -
    this.miniErrorChartMargin.top -
    this.miniErrorChartMargin.bottom;
  this.errorG = this.errorHolder
    .append("g")
    .attr(
      "transform",
      "translate(" +
  ...
  this.errorHolder
    ...
    .attr(
      ...
      "translate(" +
        15 +
        "," +
        (this.errorChartHeight / 2 + this.miniErrorChartMargin.top) +
        ")rotate(-90)"
    )
...
NN_trainer.prototype.batchAddErrorPoint = function (valuesArray) {
  ...
  d3.select(this.error_chart_el + " .error-history-line")
    ...
    .attr(
      "transform",
      "translate(" + this.error_x(-valuesArray.length) + ",0)"
    );
...
NN_trainer.prototype.rescaleErrorGraph = function () {
  ...
  this.errorG
    .select(this.error_chart_el + " .axis--y")
    .call(d3.axisLeft(this.error_y).ticks(5));

  //this.heatmapColorScale
  this.errorG.selectAll(this.error_chart_el + " .axis--y .tick text").attr(
    "fill",
    function (d) {
...
NN_trainer.prototype.calculatePrediction = function (x) {
  ...
  } else prediction = x * this.weight + this.bias;
...
NN_trainer.prototype.updateWeightAndBias = function (weight, bias, updateUI) {
  ...
  for (var i = 0; i < this.areas.length; i++) {
    ...
    errorLineValues[i] = [
      { x: this.areas[i], y: this.prices[i] },
      { x: this.areas[i], y: this.prediction[i] },
    ];
  }
  mean_delta_sum = delta_sum / this.prediction.length;

  if (updateUI) this.updateUI(mean_delta_sum, errorLineValues);
...
NN_trainer.prototype.updateUI = function (mean_delta_sum, errorLineValues) {
  ...
  if (this.error_chart_el != "") this.addErrorPoint(mean_delta_sum);
  ...
  $(this.table_el + " span#error-value").text(
    numberWithCommas(Math.round(mean_delta_sum))
  );

  // Update comment on the score
  if (mean_delta_sum < 450) {
    $(this.table_el + " span#error-value-message").html(
      "I honestly didn't know this was 'humanly' possible.."
    );
  } else if (mean_delta_sum < 500) {
    $(this.table_el + " span#error-value-message").html(
      "Hello there, superintelligent AI overlord.."
    );
  } else if (mean_delta_sum < 600) {
    $(this.table_el + " span#error-value-message").html(
      "Whoa whoa! Easy there, <a href='https://en.wikipedia.org/wiki/Yann_LeCun'>LeCun</a>!!"
    );
  } else if (mean_delta_sum < 750) {
    $(this.table_el + " span#error-value-message").text(
      "Nice! You cracked 750!"
    );
  ...
  errorLines = this.g
    .selectAll(this.svg_el + " .error-line")
  ...
  d3.select(this.svg_el + " .line").attr("d", this.valueline(this.data));
  ...
  // DATA JOIN - only really useful the first time. It adds an element for each datapoint
  predictions = this.g
    .selectAll(this.svg_el + " .prediction-dot")
  ...
  if (this.neuralNetworkGraphEl != "") this.updateNeuralNetworkGraph();
...
NN_trainer.prototype.gradientDescentStep = function (number_of_steps) {
  ...
  for (var c = 0; c < number_of_steps; c++) {
    ...
    for (var i = 0; i < this.areas.length; i++) {
      ...
      sum_for_weight =
        sum_for_weight + (this.prediction[i] - this.prices[i]) * this.areas[i];
    }
...
NN_trainer.prototype.initializeHeatmap = function () {
  ...
  this.heatmapHolder = d3
    .select(this.heatmap_el) // select the 'body' element
    .append("svg") // append an SVG element to the body
    .attr("width", this.miniGraphWidth)
    .attr("height", this.miniGraphHeight);

  this.heatmapWidth =
    +this.heatmapHolder.attr("width") -
    this.miniGraphMargin.left -
    this.miniGraphMargin.right;
  this.heatmapHeight =
    +this.heatmapHolder.attr("height") -
    this.miniGraphMargin.top -
    this.miniGraphMargin.bottom;
  this.heatmapG = this.heatmapHolder
    .append("g")
    .attr(
      "transform",
      "translate(" +
        (this.miniGraphMargin.left + 15) +
        "," +
        this.miniGraphMargin.top +
        ")"
    );
  ...
  this.heatmapY = d3
    ...
    .domain(this.biasRange)
    .range(
      d3.range(this.heatmapSideNumberOfElements).map(
        function (d) {
  ...
  this.heatmapHolder
    ...
    .attr(
      "transform",
      "translate(" +
        this.miniGraphMargin.left / 2 +
        "," +
        (this.heatmapHeight / 2 + this.miniGraphMargin.top) +
        ")rotate(-90)"
    )
...
NN_trainer.prototype.updateHeatmap = function (data) {
  var heatmapBoxSize = this.heatmapBoxSize,
    heatmapColorScale = this.heatmapColorScale;

  this.heatmap = this.heatmapG.selectAll(".heatmap-square").data(data);
  ...
  this.heatmap
    ...
    .attr(
      "fill",
      function (d, i) {
        if (d.error == 0) return this.heatmapEmptyBoxColor;
        else return heatmapColorScale(d.error);
      }.bind(this)
    );
};

NN_trainer.prototype.updateHeatmapElement = function (weight, bias, error) {
  var x = this.heatmapX(weight),
    y = this.heatmapY(bias),
    heatmapColorScale = this.heatmapColorScale;
...
NN_trainer.prototype.generateHeatmapData = function (size) {
  ...
  for (var i = 0; i < size; i++)
    for (var j = 0; j < size; j++) data.push({ x: i, y: j, error: 0 });
...
NN_trainer.prototype.initializeNeuralNetworkGraph = function () {
  this.nnGraphHolder = d3
    .select(this.neuralNetworkGraphEl) // select the 'body' element
    .append("svg") // append an SVG element to the body
    .attr("width", this.graphWidth) // make the SVG element 449 pixels wide
  ...
  this.neuralNetworkG.append("line").attrs({
    class: "arrow",
  ...
  // Weight Node
  this.weightG = this.neuralNetworkG
    .append("g")
    .attr(
      "transform",
      "translate(" +
  ...
  // Bias Node
  this.biasG = this.neuralNetworkG
    .append("g")
    .attr(
      "transform",
      "translate(" +
  ...
  this.outputNode = this.neuralNetworkG
    ...
    .attr(
      "cx",
      this.neuralNetworkWidth - nodeRadius + this.neuralNetworkMargin.left
    )
    .attr("cy", this.neuralNetworkMargin.top + this.neuralNetworkHeight / 2);
};

NN_trainer.prototype.updateNeuralNetworkGraph = function () {
  d3.select(this.neuralNetworkGraphEl + " #weightValue").text(
    this.weight.toFixed(3)
  );

  d3.select(this.neuralNetworkGraphEl + " #biasValue").text(
    this.bias.toFixed(1)
  );
};

var trainer = new NN_trainer(
  "#training-one-chart",
  "#training-one",
  [2104, 1600, 2400], // areas
  [399.9, 329.9, 369.0], // prices
  0.1, // initial weight
  150, // initial bias
  0, // x1
  0, // y1
...
var trainer2 = new NN_trainer(
  "#training-one-gd-chart",
  "#training-one-gd",
  [2104, 1600, 2400],
  [399.9, 329.9, 369.0],
  0, // initial weight
