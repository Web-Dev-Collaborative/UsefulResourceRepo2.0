...
var sigmoidGraph = function (
  ...
  this.updateSigmoidValue(
    this.containerElement,
    this.sigmoidInputElement,
    this.xDefault
  );
};
...
sigmoidGraph.prototype.initializeGraph = function () {
  var obj = this;
  this.graphHolder = d3
    .select(this.containerElement) // select the container element
  ...
  this.width =
    +this.graphHolder.attr("width") - this.margin.left - this.margin.right;
  this.height =
    +this.graphHolder.attr("height") - this.margin.top - this.margin.bottom;
  this.g = this.graphHolder
    .append("g")
    .attr(
      "transform",
  ...
  this.g
    .append("g")
    .attr("class", "y-axis")
    .call(d3.axisLeft(obj.y).tickValues([0, 0.5, 1]))
    .append("text")
    .attr("fill", "#000")
  ...
  function make_x_gridlines() {
    return d3.axisBottom(obj.x).ticks(1);
  }
  ...
  this.g
    ...
    .attr("class", "grid")
    .attr("transform", "translate(0," + this.height + ")")
    .call(make_x_gridlines().tickSize(-this.height).tickFormat(""));

  // gridlines in y axis function
  function make_y_gridlines() {
    return d3.axisLeft(obj.y).tickValues([0, 0.5, 1]);
  }
...
sigmoidGraph.prototype.updateGraph = function () {
  ...
  var svg = d3
    .select(this.containerElement + " svg")
    .attr("width", this.graphWidth)
    .attr("height", 160);
  ...
  d3.select(".sigmoid-line").attr("d", line);
};
...
sigmoidGraph.prototype.attachEventsToSlider = function () {
  ...
  $(this.sliderElement).on("input change", function () {
    self.updateSigmoidValue(
      self.containerElement,
      self.sigmoidInputElement,
      this.value
    );
  });
...
sigmoidGraph.prototype.updateSigmoidValue = function (
  containerElement,
  sliderElement,
  inputValue
) {
  // Update Text values
  d3.selectAll(this.sigmoidInputElement).text(inputValue);
  d3.select(this.sigmoidFormulaInputElement).text(
    parseFloat(inputValue).toFixed(2)
  );
  // Update formula result
  d3.select(this.sigmoidResult).text(sigmoid(inputValue).toFixed(11));
  d3.select(this.sigmoidGraphResult).text(sigmoid(inputValue).toFixed(11));

  // Update graph
  d3.select(this.containerElement + " .value-point").attr(
    "transform",
    "translate(" + this.x(inputValue) + "," + this.y(sigmoid(inputValue)) + ")"
  );

  // Update graph dot label
  d3.select(this.containerElement + " .sigmoid-value-text").text(
    sigmoid(inputValue).toFixed(11)
  );
};
