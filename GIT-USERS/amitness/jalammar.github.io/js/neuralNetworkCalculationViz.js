...
neuralNetworkCalculationViz.prototype.drawGraph = function () {
  ...
  var inputLayerX = this.neuralNetworkMargin.left + this.nodeRadius, // X value of input layer
    ...
    outputLayerX =
      this.neuralNetworkWidth - this.nodeRadius + this.neuralNetworkMargin.left,
  ...
  // Input layer
  this.getYCoordinateOfNodeInInputLayer = d3
    .scaleLinear()
  ...
  this.getYCoordinateOfNodeInOutputLayer = d3
    .scaleLinear()
  ...
  this.inputLayerCoordinates.push({
    x: inputLayerX,
    y: grapher.getYCoordinateOfNodeInInputLayer(this.inputNodeCount),
    index: this.inputNodeCount,
    type: "bias",
  });

  // Bias layer
  this.outputLayerCoordinates = d3
    .range(this.outputNodeCount)
    .map(function (i) {
  ...
  var weightMarginLeft =
      grapher.nodeRadius +
      grapher.nodeRadius * grapher.weightNodeWidthRatio -
      15,
  ...
  this.weightLayerXCoordinates = d3
    .range(this.outputNodeCount)
    .map(function (i) {
  ...
  for (var j = 0; j < this.outputNodeCount; j++)
    for (var i = 0; i < this.inputLayerNodeCount; i++) {
      var opposite =
          this.inputLayerCoordinates[i].y - this.outputLayerCoordinates[j].y,
        adjacent = biasLayerX - inputLayerX,
        angle = Math.atan(opposite / adjacent),
        yCoordinate =
          this.inputLayerCoordinates[i].y -
          Math.tan(angle) * (this.weightLayerXCoordinates[j] - inputLayerX);
...
neuralNetworkCalculationViz.prototype.updateInputNodesText = function (
  newinputs
) {
    ...
    .on("mouseover", function (d) {
      ...
      for (var i = 0; i < grapher.layerInputs[1].length; i++) {
        rows =
          ...
          (i + 1) +
          "</td><td class='importance-20'>" +
          roundPrecision(grapher.layerInputs[1][i][d.index], 8) +
          "</td></tr>";
      ...
      rows =
        rows +
        "<tr class='total-row'><td class='importance-10'>Sum:</td><td class='importance-30'>" +
        roundPrecision(total, 8) +
        "</td></tr>";
      rows = rows + "</table>";
...
neuralNetworkCalculationViz.prototype.graphLayerInputs = function (
  layerInputsArray,
  layerNumber
) {
  ...
  layerInputselements
    ...
    .text(function (d) {
      ...
      else return d.value.toExponential(2);
    });
};

neuralNetworkCalculationViz.prototype.updateLayerInputs = function (
  layerInputsArray,
  layerNumber
) {
...
neuralNetworkCalculationViz.prototype.graphSoftmaxOutputs = function (
  softmaxOutputs
) {
  var grapher = this;
  var data = [];
...
neuralNetworkCalculationViz.prototype.graphBiasToSoftmaxArrows = function (
  data
) {
  var inputToBiasLines = this.neuralNetworkG
...
neuralNetworkCalculationViz.prototype.graphSoftmaxToOutputArrows = function (
  data
) {
  var inputToBiasLines = this.neuralNetworkG
...
var weights = [[-0.11510573], [0.19847003]];
