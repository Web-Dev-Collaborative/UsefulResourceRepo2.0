 ...
 */
var shallowNeuralNetworkGrapher = function (
  inputNodeCount,
  outputNodeCount,
  svgElement,
  analyticsCategory
...
shallowNeuralNetworkGrapher.prototype.updateNodeCount = function (
  nodeType,
  newValue
) {
...
shallowNeuralNetworkGrapher.prototype.drawGraph = function () {
  ...
  this.inputLayerCoordinates = d3.range(this.inputNodeCount).map(function (i) {
    var y =
      grapher.neuralNetworkMargin.top +
      ((i + 1) * grapher.neuralNetworkHeight) / (grapher.inputNodeCount + 1);
  ...
  this.biasLayerCoordinates = d3.range(this.outputNodeCount).map(function (i) {
    var y =
      grapher.neuralNetworkMargin.top +
      ((i + 1) * grapher.neuralNetworkHeight) / (grapher.outputNodeCount + 1);
  ...
  var weightMarginLeft =
    ...
    weightRegion =
      biasLayerX - inputLayerX - weightMarginLeft - weightMarginRight;
...
shallowNeuralNetworkGrapher.prototype.graphWeightNodes = function (data) {
  ...
  // UPDATE old elements present in new data.
  groupElements.attr("class", "weight-group").attr("transform", function (d) {
    return "translate(" + d.x + "," + d.y + ")";
  });
...
shallowNeuralNetworkGrapher.prototype.graphBiasNodes = function (data) {
  // JOIN
  var biasNodes = this.neuralNetworkG
    .selectAll(this.svgElement + " .bias-group")
    .data(data);

  // EXIT old elements not present in new data.
  biasNodes.exit().attr("class", "exit").remove();

  // UPDATE old elements present in new data.
  biasNodes.attr("class", "bias-group").attr(
    "transform",
    function (d) {
...
shallowNeuralNetworkGrapher.prototype.graphBiasToSoftmaxArrows = function (
  data
) {
  var inputToBiasLines = this.neuralNetworkG
...
shallowNeuralNetworkGrapher.prototype.graphSoftmaxToOutputArrows = function (
  data
) {
  var inputToBiasLines = this.neuralNetworkG
...
var softmaxNNExample = new shallowNeuralNetworkGrapher(
  2,
  2,
  "#shallow-neural-network-graph",
  ...
  $(".input-number").change(function () {
    ...
    if (fieldName == "quant[1]") {
      ...
      ga(
        "send",
        "event",
        softmaxNNExample.analyticsCategory,
        "Set number of",
        "Features",
        valueCurrent
      );
