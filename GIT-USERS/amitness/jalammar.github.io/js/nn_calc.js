(function (global, factory) {
  ...
  function graphInputNodes(data) {
    var grapher = this;
    // JOIN
    var inputGroups = this.neuralNetworkG
      .selectAll(this.svgElement + " .input-group")
      .data(data);

    // EXIT old elements not present in new data.
    inputGroups.exit().attr("class", "exit").remove();

    // UPDATE old elements present in new data.
    inputGroups.attr("class", "input-group").attr(
      "transform",
      function (d) {
    ...
    inputGroups.select("#input-name").text(function (d) {
      return "X" + (d.index + 1);
    });

    // ENTER new elements present in new data.
    var inputs = inputGroups
      .enter()
      .append("g")
      ...
      .on("mouseover", function (d) {
        ...
        d3.selectAll(grapher.svgElement + " line.input-" + d.index).classed(
          "nn-arrow-highlighted",
          true
        );

        // Highlight the weights connected to the node
        var weightGroups = d3.selectAll(
          grapher.svgElement + " .input-" + d.index + "-weight"
        );
        weightGroups.selectAll("ellipse").classed("nn-node-highlighted", true);
        var layerInputs = d3
          .selectAll(
            grapher.svgElement + " .layer-1-input-groups .input-" + d.index
          )
      ...
      .on("mouseout", function (d) {
        ...
        d3.selectAll(grapher.svgElement + " .input-" + d.index).classed(
          "nn-arrow-highlighted",
          false
        );
        ...
        weightGroups.selectAll("ellipse").classed("nn-node-highlighted", false);
  ...
  function updateInputNodesText(newinputs) {
    var inputTexts = this.neuralNetworkG.selectAll(
      this.svgElement + " .input-group .node-text"
    );
  ...
  function getInputNodeCoordinates(
    inputNodeCount,
    inputLayerX,
    getYCoordinateOfNodeInInputLayer
  ) {
    var inputLayerCoordinates = d3.range(inputNodeCount).map(
      function (i) {
    ...
    inputLayerCoordinates.push({
      x: inputLayerX,
      y: getYCoordinateOfNodeInInputLayer(inputNodeCount),
      index: inputNodeCount,
      type: "bias",
    });
  ...
  function graphWeightNodes(data) {
    ...
    // JOIN
    var groupElements = this.neuralNetworkG
      .selectAll(this.svgElement + " .weight-group")
      .data(data);

    // EXIT old elements not present in new data.
    groupElements.exit().attr("class", "exit").remove();
    ...
    groupElements.select("#weight0Value").text(function (d) {
    ...
    var groups = groupElements
      ...
      .on("mouseover", function (d) {
        ...
        g.select("ellipse").transition().duration(50).attr("rx", 50);
      ...
      .on("mouseout", function (d) {
        var g = d3.select(this);
        g.select("ellipse").transition().duration(50).attr("rx", 22);
  ...
  function getWeightNodeCoordinates(
    ...
    inputNodeCount,
    outputNodeCount,
    inputLayerCoordinates,
    weightLayerXCoordinates,
    ...
    for (var j = 0; j < outputNodeCount; j++)
      for (var i = 0; i < inputLayerNodeCount; i++) {
        var opposite = inputLayerCoordinates[i].y - outputLayerCoordinates[j].y,
          ...
          yCoordinate =
            inputLayerCoordinates[i].y -
            Math.tan(angle) * (weightLayerXCoordinates[j] - inputLayerX);
  ...
  function graphNode(data) {
    ...
    // JOIN
    var inputGroups = this.neuralNetworkG
      .selectAll(this.svgElement + " .output-group")
      .data(data);
    ...
    // UPDATE old elements present in new data.
    inputGroups.attr("class", "output-group").attr(
      "transform",
      function (d) {
        return "translate(" + d.x + "," + d.y + ")";
      }.bind(this)
    ...
    inputGroups.select("#output-name").text(function (d) {
      return "Y" + (d.index + 1);
    });
    ...
    var inputs = inputGroups
      ...
      .on("mouseover", function (d) {
        // Highlight the lines connected to this node
        d3.selectAll(grapher.svgElement + " line.output-" + d.index).classed(
          "nn-arrow-highlighted",
          true
        );
        ...
        // Highlight the weights connected to the node
        var weightGroups = d3.selectAll(
          grapher.svgElement + " .output-" + d.index + "-weight"
        );
        weightGroups.selectAll("ellipse").classed("nn-node-highlighted", true);

        var layerInputs = d3
          .selectAll(
            grapher.svgElement + " .layer-1-input-groups .output-" + d.index
          )
          .classed("layer-input-value-highlighted", true);
        var xValue = d.x;

        // Set up the tooltip to show the inputs and their sum
        var rows = '<table class="input-calculation">',
          total = 0,
          classToColorNumber = "weight-layer-input";
        rows =
          rows +
          "<tr><th style='text-align:center'>#</th><th>input value</th></tr> ";
        for (var i = 0; i < grapher.layerInputs[1].length; i++) {
          ...
          rows =
            rows +
            "<tr><td class='importance-10'>" +
            (i + 1) +
            "</td><td class='number " +
            classToColorNumber +
            "'>" +
            roundPrecision(grapher.layerInputs[1][i][d.index], 5) +
            "</td></tr>";
        ...
        rows =
          rows +
          "<tr class='total-row'><td class='importance-10'>Sum:</td><td class='sum-value number'>" +
          roundPrecision(total, 5) +
          "</td></tr>";
        rows = rows + "</table>";
        ...
        grapher.tooltipDiv.transition().duration(100).style("opacity", 1);
        grapher.tooltipDiv
          .html(rows)
          .style("left", d3.event.pageX + "px")
      ...
      .on("mouseout", function (d) {
        // Stop highlighting the lines connected to this node
        d3.selectAll(grapher.svgElement + " .output-" + d.index).classed(
          "nn-arrow-highlighted",
          false
        );
        ...
        grapher.tooltipDiv.transition().duration(200).style("opacity", 0);
      });
  ...
  function updateOutputNodesText() {
    var outputTexts = this.neuralNetworkG.selectAll(
      this.svgElement + " .output-group .node-text"
    );
    outputTexts.text(
      function (d) {
  ...
  function getOutputNodeCoordinates(
    outputNodeCount,
    biasLayerX,
    getYCoordinateOfNodeInOutputLayer
  ) {
    var outputLayerCoordinates = d3.range(outputNodeCount).map(
      function (i) {
  ...
  function graphLayerInputs(layerInputsArray, layerNumber) {
    var layerInputs = layerInputsArray[layerNumber],
      layerInputsObjects = [];
    ...
    var layerInputGroups = this.neuralNetworkG
      .selectAll(this.svgElement + " .layer-" + layerNumber + "-input-groups")
    ...
    var grapher = this,
      xCoordinate = 225,
      offset = 4;
    var layerInputselements = layerInputGroups
      .enter()
      .append("g")
      .attrs({
        ...
        transform: function (d) {
          ...
          var yRelativeToInputNode =
              Math.tan(grapher.lineAngles[d.input][d.output]) *
              (xCoordinate - grapher.inputLayerX),
            // Now to transform the relative Y to absolute Y
            yCoordinate =
              grapher.inputLayerCoordinates[d.input].y -
              yRelativeToInputNode -
              offset,
            opposite = grapher.outputLayerCoordinates[d.output].y - yCoordinate,
            adjacent =
              grapher.lineAngles[d.input][d.output] != 0
                ? opposite / Math.tan(-grapher.lineAngles[d.input][d.output])
                : 0.0001,
      ...
      .attrs({
        class: function (d) {
          ...
          return (
            "layer-" +
            layerNumber +
            "-inputs layer-input-value output-" +
            d.output +
            " input-" +
            d.input +
            " " +
  ...
  function updateLayerInputs(layerInputsArray, layerNumber) {
    var layerInputtext = this.neuralNetworkG.selectAll(
      this.svgElement +
        " .layer-" +
        layerNumber +
        "-input-groups layer-" +
  ...
  function graphSoftmax(data) {
    var softmax = this.neuralNetworkG
      .selectAll(this.svgElement + " .softmax")
      .data([data]);

    var softmaxGroup = softmax
      .enter()
      .append("g")
      .attr("class", "softmax")
      .attr("transform", function (d) {
    ...
    var activate = function (d) {
      if (viz.layerValues[1]) {
        // Set up the tooltip to show the inputs and their sum
        var rows = '<table class="softmax-calculation">',
          total = 0;
        rows =
          rows +
          "<tr>" +
        ...
        var sum_term = viz.layerValues[1]
          .map(function (d) {
            return "e^{" + numberFormatter(d, 2) + "}";
          })
          .join(" + ");
        var exponential_terms = [];
        var softMaxTopResult = viz.softmaxOutputs.indexOf(
          d3.max(viz.softmaxOutputs)
        );
        for (var i = 0; i < viz.layerValues[1].length; i++) {
          exponential_terms[i] =
            " e^{" + numberFormatter(viz.layerValues[1][i], 2) + "}";
          var eq = katex.renderToString(
            "\\frac{" + exponential_terms[i] + "}{" + sum_term + "}"
          );
          ...
          var node_row =
            "<tr>" +
            "<td class='input'>" +
            numberFormatter(viz.layerValues[1][i], 2) +
            "</td>" +
            ...
            highlight_cell +
            "'>" +
            numberFormatter(viz.softmaxOutputs[i], 2) +
            "</td>" +
            "<td class='class-name " +
            highlight_cell +
            "'>" +
        ...
        viz.tooltipDiv.transition().duration(100).style("opacity", 1);
        viz.tooltipDiv
          .html(rows)
          .style("left", d3.event.pageX + "px")
          .style("top", d3.event.pageY + "px");
        ...
        var html =
          "Hover-over or tap one of the row in the table, then you'll see the softmax calculation here.";
    ...
    var deactivate = function (d) {
      viz.tooltipDiv.transition().duration(200).style("opacity", 0);
    };
  ...
  function graphSoftmaxOutputs(softmaxOutputs) {
    ...
    var softmaxOutputGroups = this.neuralNetworkG
      .selectAll(this.svgElement + " .softmax-output-group")
      .data(data);

    softmaxOutputGroups
      .select("text")
      .attr("class", function (d) {
        if (d.value == maxValueIndex) return "highlighted-softmax-output";
        else return "dim-softmax-output";
      })
      .text(function (d) {
        return numberFormatter(d.value * 100, 2) + "%";
      });

    var softmaxOutputElements = softmaxOutputGroups
      .enter()
      .append("g")
      .attrs({
        ...
        transform: function (d) {
          return (
            "translate(" +
            (grapher.softmaxOutputsX - 8) +
            ", " +
            (grapher.getYCoordinateOfNodeInOutputLayer(d.index) + 5) +
            ")"
          );
        },
  ...
  function highlightTopSoftmaxOutput() {
    var highlightOrDimClass = "",
      topClassIndex = -1;
    if (this.softmaxOutputs) {
      topClassIndex = this.softmaxOutputs.indexOf(d3.max(this.softmaxOutputs));
    }

    var softmaxOutputClassName = this.neuralNetworkG.selectAll(
      this.svgElement + " .softmax-output-class-name"
    );
    softmaxOutputClassName.attr("class", function (d, i) {
      var classValue = "softmax-output-class-name ";

      if (d.outputNode == topClassIndex)
        classValue = classValue + "highlighted-softmax-output";
      else classValue = classValue + "dim-softmax-output";
  ...
  softmax = function (input) {
    if (!(input instanceof Array))
      throw TypeError(
        "Input passed into softmax() is not an array. softmax() expects an array. Input: " +
          input,
        "nnVizUtils.js"
      );
    ...
    for (var i = 0; i < input.length; i++) {
      if (!(typeof input[i] == "number")) {
        throw TypeError(
          "An element in the array passed to softmax() is not a number. softmax() expects an array of numbers. Input: " +
            input[i],
          "nnVizUtils.js"
        );
      }
  ...
  function graphSigmoid(data) {
    var sigmoid = this.neuralNetworkG
      .selectAll(this.svgElement + " .sigmoid")
      .data([data]);

    var sigmoidGroup = sigmoid
      .enter()
      .append("g")
    ...
    var activate = function (d) {
      ...
      if (viz.layerValues[1]) {
        ...
        var node_row =
          ...
          "</td>" +
          "<td>" +
          numberFormatter(viz.sigmoidOutput, 2) +
          "</td>" +
          "</tr>";
        ...
        var html =
          "Hover-over or tap one of the row in the table, then you'll see the sigmoid calculation here.";
  ...
  function graphSigmoidOutput(sigmoidOutput) {
    ...
    var sigmoidOutputGroups = this.neuralNetworkG
      .selectAll(this.svgElement + " .sigmoid-output-group")
      .data(data);
    ...
    var sigmoidOutputElements = sigmoidOutputGroups
      .enter()
      .append("g")
      .attrs({
  ...
  function getInputToBiasLinesData(
    inputLayerNodeCount,
    outputNodeCount,
    inputLayerCoordinates,
    outputLayerCoordinates
  ...
  function getBiasToSoftmaxLinesData(
    outputNodeCount,
    outputLayerCoordinates,
    activationLayerX,
    nodeRadius
  ...
  function graphInputToBiasLines(data) {
    var inputToBiasLines = this.neuralNetworkG
      .selectAll(this.svgElement + " .input-to-bias-line")
      .data(data);
    ...
    inputToBiasLines
      ...
      .attrs({
        class: function (d) {
          return (
            classes + " input-" + d.inputIndex + " output-" + d.outputIndex
          );
        },
        x1: function (d) {
          return d.x1;
  ...
  function graphBiasToSoftmaxArrows(data) {
    var inputToBiasLines = this.neuralNetworkG
      .selectAll(this.svgElement + " .bias-to-softmax-line")
      .data(data);
  ...
  function graphSoftmaxToOutputArrows(data) {
    var inputToBiasLines = this.neuralNetworkG
      .selectAll(this.svgElement + " .softmax-to-output-line")
      .data(data);
    ...
    var softmaxOutputClassName = this.neuralNetworkG
      .selectAll(this.svgElement + " .softmax-output-class-name")
      .data(data);

    softmaxOutputClassName.exit().remove();
  ...
  function getLayerXScale(
    ...
    var returnFunction;
    if (numberOfLayers == 1) {
      // If only one node, draw it in the center
      returnFunction = function () {
        return layersRegionWidth / 2 + marginLeft;
      };
    } else {
      // Otherwise, separate the vertical space linearly
      returnFunction = d3
        .scaleLinear()
  ...
  function getNodeYCoordinate(
    numberOfNodesInLayer,
    chartAreaHeight,
    marginTop,
    nodeRadius
  ) {
    var returnFunction;
    if (numberOfNodesInLayer == 1) {
      // If only one node, draw it in the center
      returnFunction = function () {
      ...
      returnFunction = d3
        ...
        .range([marginTop + nodeRadius, chartAreaHeight - nodeRadius]);
    }
    return returnFunction;
  }
  ...
  var input = {
    getNodeRenderingDetails: function (layer, properties) {
      ...
      if (layer.bias == true) inputAndBiasNodeCount++;
      console.log("inputAndBiasNodeCount", inputAndBiasNodeCount);

      // Create a scale to determine the Y coordinate of each node
      var layerNodesYScale = getNodeYCoordinate(
        inputAndBiasNodeCount,
        properties.activeRegionHeight,
        properties.margins.top,
        properties.nodeRadius
      );

      var layerCoordinates = getInputNodeCoordinates$1(
        layer.nodes,
        layer.x,
        layerNodesYScale
      );
    ...
    draw: function (layer, properties, activeRegionSVGGroup) {
      ...
      var inputGroups = activeRegionSVGGroup
        .selectAll(properties.containerElement + " .input-group")
        .data(data);
      ...
      var inputs = inputGroups
        ...
        .on("mouseover", function (d) {
          // Highlight the lines connected to this node
          d3.selectAll(
            properties.containerElement + " line.input-" + d.index
          ).classed("nn-arrow-highlighted", true);

          // Highlight the weights connected to the node
          var weightGroups = d3.selectAll(
            properties.containerElement + " .input-" + d.index + "-weight"
          );
          weightGroups
            .selectAll("ellipse")
        ...
        .on("mouseout", function (d) {
          // Stop highlighting the lines connected to this node
          d3.selectAll(
            properties.containerElement + " .input-" + d.index
  ...
  function getInputNodeCoordinates$1(
    inputNodeCount,
    inputLayerX,
    getYCoordinateOfNodeInInputLayer
  ) {
  ...
  var block = {
    ...
    draw: function (layer, properties, activeRegionSVGGroup) {
      ...
      var nodeGroups = activeRegionSVGGroup
        .selectAll(properties.containerElement + " .layer-" + layer.index)
  ...
  var nodes = {
    getNodeRenderingDetails: function (layer, properties) {
      ...
      var layerCoordinates = getNodeCoordinates(
        layer.nodes,
        layer.x,
        layerNodesYScale
      );
    ...
    draw: function (
      ...
      // Each node has an SVG group of its own. In that group is an SVG circle and SVG text label
      var nodeGroups = activeRegionSVGGroup
        .selectAll(containerElement + " .layer-" + layer.index)
        .data(data);

      // EXIT old elements not present in new data.
      nodeGroups.exit().attr("class", "exit").remove();
      ...
      nodeGroups.select("#output-name").text(function (d) {
        return "Y" + (d.index + 1);
      });

      // ENTER new elements present in new data.
      var inputs = nodeGroups
        .enter()
        .append("g")
        .attr("class", ".layer-" + layer.index + " layer-group")
        .attr("transform", function (d) {
        ...
        .on("mouseover", function (d) {
          if (layerInputs != "") {
            // Highlight the lines connected to this node
            d3.selectAll(
              activeRegionSVGGroup + " line.output-" + d.index
            ).classed("nn-arrow-highlighted", true);

            // Highlight the weights connected to the node
            var weightGroups = d3.selectAll(
              activeRegionSVGGroup + " .output-" + d.index + "-weight"
            );
            weightGroups
              .selectAll("ellipse")
              .classed("nn-node-highlighted", true);
            ...
            for (var i = 0; i < layerInputs[1].length; i++) {
              ...
              rows =
                ...
                classToColorNumber +
                "'>" +
                roundPrecision(layerInputs[1][i][d.index], 5) +
                "</td></tr>";
        ...
        .on("mouseout", function (d) {
          if (layerInputs != "") {
            // Stop highlighting the lines connected to this node
            d3.selectAll(activeRegionSVGGroup + " .output-" + d.index).classed(
              "nn-arrow-highlighted",
              false
            );
  ...
  var neuralNetworkCalculationViz = function (
    inputs,
    inputNodeCount,
    outputNodeCount,
    activation,
  ...
  neuralNetworkCalculationViz.prototype.updateInputNodesText =
    updateInputNodesText;
  neuralNetworkCalculationViz.prototype.getInputNodeCoordinates =
    getInputNodeCoordinates;
  // Weight
  neuralNetworkCalculationViz.prototype.graphWeightNodes = graphWeightNodes;
  neuralNetworkCalculationViz.prototype.getWeightNodeCoordinates =
    getWeightNodeCoordinates;
  // Node / output
  neuralNetworkCalculationViz.prototype.graphBiasNodes = graphNode;
  neuralNetworkCalculationViz.prototype.updateOutputNodesText =
    updateOutputNodesText;
  neuralNetworkCalculationViz.prototype.getOutputNodeCoordinates =
    getOutputNodeCoordinates;
  // Layer inputs
  ...
  neuralNetworkCalculationViz.prototype.graphSoftmaxOutputs =
    graphSoftmaxOutputs;
  neuralNetworkCalculationViz.prototype.highlightTopSoftmaxOutput =
    highlightTopSoftmaxOutput;
  // Sigmoid
  ...
  // Connection lines
  neuralNetworkCalculationViz.prototype.getInputToBiasLinesData =
    getInputToBiasLinesData;
  neuralNetworkCalculationViz.prototype.graphInputToBiasLines =
    graphInputToBiasLines;
  neuralNetworkCalculationViz.prototype.getBiasToSoftmaxLinesData =
    getBiasToSoftmaxLinesData;
  neuralNetworkCalculationViz.prototype.graphBiasToSoftmaxArrows =
    graphBiasToSoftmaxArrows;
  neuralNetworkCalculationViz.prototype.getSoftmaxtoOutputLinesData =
    getSoftmaxtoOutputLinesData;
  neuralNetworkCalculationViz.prototype.graphSoftmaxToOutputArrows =
    graphSoftmaxToOutputArrows;

  neuralNetworkCalculationViz.prototype.updateNodeCount = function (
    nodeType,
    newValue
  ) {
  ...
  neuralNetworkCalculationViz.prototype.initializeGraph = function (
    ...
    var nnGraphHolder = d3
      .select(containerElement) // select the 'body' element
    ...
    (this.neuralNetworkWidth =
      +nnGraphHolder.attr("width") -
      this.properties.margins.left -
      this.properties.margins.right),
      (this.neuralNetworkHeight =
        +nnGraphHolder.attr("height") -
        this.properties.margins.top -
  ...
  neuralNetworkCalculationViz.prototype.initializeGraphOld = function () {
    this.nnGraphHolder = d3
      .select(this.svgElement) // select the 'body' element
      .append("svg") // append an SVG element to the body
    ...
    (this.neuralNetworkMargin = { top: 10, right: 10, bottom: 10, left: 10 }),
      (this.neuralNetworkWidth =
        +this.nnGraphHolder.attr("width") -
        this.neuralNetworkMargin.left -
        this.neuralNetworkMargin.right),
      (this.neuralNetworkHeight =
        +this.nnGraphHolder.attr("height") -
        this.neuralNetworkMargin.top -
        this.neuralNetworkMargin.bottom),
  ...
  neuralNetworkCalculationViz.prototype.drawGraph = function (inputs) {
    ...
    var layers = inputs.layers,
      ...
      // Linear scale between this.neuralNetworkMargin.left and this.neuralNetworkWidth + this.neuralNetworkMargin.left + this.properties.outputRegionWidth
      layerXScale = getLayerXScale(
        numberOfLayers,
        this.properties.activeRegionWidth,
        this.properties.margins.left,
        this.properties.outputRegionWidth,
        this.properties.nodeRadius
      );
    ...
    for (var i = 0; i < numberOfLayers; i++) {
      ...
      layers[i].renderer =
        typeof layers[i].type !== "undefined"
          ? renderers[layers[i].type]
          : renderers["defaultLayer"];
    ...
    for (var j = 0; j < numberOfLayers; j++) {
      layers[j].nodeRenderingDetails = layers[
        j
      ].renderer.getNodeRenderingDetails(layers[j], this.properties);
    }
    ...
    for (var m = 0; m < numberOfLayers; m++) {
      layers[m].renderer.draw(
        layers[m],
        this.properties,
        this.activeRegionSVGGroup,
        this.layerInputs,
        this.tooltipDiv
      );
    }
  ...
  neuralNetworkCalculationViz.prototype.getNodeRenderingDetails = function (
    layer,
    properties
  ) {
    //if( )
    switch (layer.type) {
      case "input": // Special consideration for bias nodes
        layer.nodeRenderingDetails = input.getNodeRenderingDetails(
          layer,
          properties
        );
  ...
  neuralNetworkCalculationViz.prototype.drawGraphOld = function () {
    ...
    this.getYCoordinateOfNodeInInputLayer = getNodeYCoordinate(
      this.inputNodeCount + 1,
      this.neuralNetworkHeight,
      this.neuralNetworkMargin.top,
      this.nodeRadius
    );

    this.getYCoordinateOfNodeInOutputLayer = getNodeYCoordinate(
    ...
    this.inputLayerCoordinates = this.getInputNodeCoordinates(
      this.inputNodeCount,
      inputLayerX,
    ...
    //  2.2 - Output layer
    this.outputLayerCoordinates = this.getOutputNodeCoordinates(
      this.outputNodeCount,
      biasLayerX,
    ...
    this.weightLayerXCoordinates = this.getWeightLayerXCoordinates(
      grapher.nodeRadius,
      grapher.weightNodeWidthRatio,
      this.outputNodeCount,
      inputLayerX
    );

    // Weight Coordinates
    var weightNodeData = this.getWeightNodeCoordinates(
      this.inputLayerNodeCount,
      this.inputNodeCount,
      this.outputNodeCount,
      this.inputLayerCoordinates,
      this.weightLayerXCoordinates,
      this.outputLayerCoordinates,
      biasLayerX,
      inputLayerX
    );
    ...
    this.inputToBiasLines = this.getInputToBiasLinesData(
      this.inputLayerNodeCount,
      this.outputNodeCount,
      this.inputLayerCoordinates,
      this.outputLayerCoordinates
    ...
    this.BiasToSoftmaxLines = this.getBiasToSoftmaxLinesData(
      this.outputNodeCount,
      this.outputLayerCoordinates,
      activationLayerX,
      grapher.nodeRadius
    ...
    this.softmaxtoOutputLines = this.getSoftmaxtoOutputLinesData(
      this.outputNodeCount,
      activationLayerX,
      grapher.nodeRadius,
      this.outputLayerCoordinates,
    ...
    var biasNodeY =
      this.neuralNetworkMargin.top +
      this.neuralNetworkHeight / 2 -
      this.nodeRadius;
  };

  neuralNetworkCalculationViz.prototype.graphActivation = function (
    activationFunction,
    activationLayerXCoordinate
  ) {
    if (activationFunction == "softmax") {
      var softmaxCoordinates = {
        x: activationLayerXCoordinate,
        y: this.neuralNetworkMargin.top,
      };
  ...
  neuralNetworkCalculationViz.prototype.feedforward = function (layerNumber) {
    ...
    for (var i = 0; i < this.weightsAndBiasWeights.length; i++)
      for (var j = 0; j < this.weightsAndBiasWeights[i].length; j++) {
        ...
        this.layerInputs[layerNumber][i][j] =
          this.weightsAndBiasWeights[i][j] * this.inputs[i];

        // Calculate the output nodes
        if (i == 0) this.layerValues[1][j] = 0;

        this.layerValues[layerNumber][j] =
          this.layerValues[1][j] + this.layerInputs[layerNumber][i][j];
      }
  };

  neuralNetworkCalculationViz.prototype.initializeTable = function (
    trainingSet
  ) {
    var viz = this;
    var table = d3
      .select(this.tableElement)
      .append("table")
    ...
    var columns = [
      {
        ...
        html: function (d, i) {
          return (
            '<input type="radio" name="person"  ' + 'class="radio_' + i + '">'
          );
        },
      },
    ...
    table
      ...
      .on("mouseout", function (d) {
        if (viz.selectedPerson) viz.setInput(viz.selectedPerson);
      })
      .on("click", function (d) {
        d3.select("radio_" + d.age + "_" + d.sex).attr("checked", "true");
      })
      .selectAll("td")
      .data(function (row, i) {
  ...
  neuralNetworkCalculationViz.prototype.getWeightLayerXCoordinates = function (
    nodeRadius,
    weightNodeWidthRatio,
    outputNodeCount,
    inputLayerX
  ) {
  ...
  neuralNetworkCalculationViz.prototype.updateActivation = function (
    activationInput,
    activation
  ) {
  ...
  var nnCalculationViz = new neuralNetworkCalculationViz(
    false,
    2,
    1,
    "sigmoid",
    ...
    weights,
    biases,
    "#neural-network-calculation-table",
    trainingSet,
    classes
  );
