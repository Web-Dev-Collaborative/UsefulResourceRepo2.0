  ...
  var _defaults = {
    ...
    _config = {
      oldIE:
        ...
        navigator.appVersion.indexOf("MSIE 9.") > -1, // Check if is a old IE.
      ...
      overlay: {
        ...
        together: [
          ...
          "slit",
          "slip",
        ], // Animation together (overlay and modal).
      },
      modal: {
    ...
    _private = {
      set: function (val) {
        ...
        if (val && val.zIndex === "auto") {
          for (
            var zIndex = 0,
              ...
              xLen = elements.length;
            x < xLen;
            x += 1
          ) {
            var value = window
              .getComputedStyle(elements[x])
        ...
        this.cb.push({
          settings:
            _config.oldIE && typeof cbExtendObjects !== "undefined"
              ? cbExtendObjects({}, _defaults, val)
              : Object.assign({}, _defaults, val),
        });

        if (this.cb[this.item].settings.overlayEffect === "auto") {
          this.cb[this.item].settings.overlayEffect =
            this.cb[this.item].settings.effect;
        }
      },
      get: function () {
      ...
      init: function () {
        ...
        document.documentElement.classList.add(
          "custombox-open-" + this.cb[this.item].settings.overlayEffect
        );

        // Add class perspective.
        if (
          _config.overlay.perspective.indexOf(
            this.cb[this.item].settings.overlayEffect
          ) > -1
        ) {
          this.cb[this.item].scroll =
        ...
        // Create loading.
        if (
          this.cb[this.item].settings.loading &&
          this.cb[this.item].settings.loading.parent
        ) {
          this.built("loading");
        }
      ...
      built: function (item) {
        ...
        switch (item) {
          ...
          case "overlay":
            ...
            cb.overlay.classList.add(
              "custombox-overlay-" + cb.settings.overlayEffect
            );
            ...
            // Add class perspective.
            if (
              _config.overlay.perspective.indexOf(cb.settings.overlayEffect) >
                -1 ||
              _config.overlay.together.indexOf(cb.settings.overlayEffect) > -1
            ) {
            ...
            if (
              ...
              cb.overlay.style.transitionDuration =
                cb.settings.overlaySpeed + "ms";
            }
          ...
          case "modal":
            ...
            cb.wrapper.classList.add(
              "custombox-modal-wrapper-" + cb.settings.effect
            );
            ...
            cb.container.classList.add(
              "custombox-modal-container-" + cb.settings.effect
            );
            cb.container.style.zIndex = cb.settings.zIndex + 4;

            if (
              _config.modal.position.indexOf(cb.settings.effect) > -1 &&
              cb.settings.animation === null
            ) {
              // Defaults.
          ...
          case "loading":
            ...
            for (
              var i = 0, t = this.cb[this.item].settings.loading.parent.length;
              i < t;
              i++
            ) {
              wrapper.classList.add(
                this.cb[this.item].settings.loading.parent[i]
              );
            }
            ...
            if (this.cb[this.item].settings.loading.childrens) {
              for (
                var e = 0,
                  te = this.cb[this.item].settings.loading.childrens.length;
                e < te;
                e++
              ) {
                ...
                for (
                  var r = 0,
                    tr =
                      this.cb[this.item].settings.loading.childrens[e].length;
                  r < tr;
                  r++
                ) {
                  tmp.classList.add(
                    this.cb[this.item].settings.loading.childrens[e][r]
                  );
                }
      ...
      load: function () {
        ...
        if (
          cb.settings.target !== null &&
          Array.isArray(cb.settings.position)
        ) {
          if (
            cb.settings.target.charAt(0) === "#" ||
            (cb.settings.target.charAt(0) === "." &&
              cb.settings.target.charAt(1) !== "/")
          ) {
      ...
      size: function () {
        ...
        if (cb.size + 60 >= window.innerWidth) {
          ...
          for (
            var i = 0,
              elements = cb.content.querySelectorAll(":scope > *"),
              t = elements.length;
            i < t;
            i++
          ) {
        ...
        // Top.
        if (
          cb.content.offsetHeight >= window.innerHeight &&
          cb.settings.width !== "full"
        ) {
          ...
          switch (cb.settings.position[1].trim()) {
            ...
            default:
              result =
                window.innerHeight / 2 - cb.content.offsetHeight / 2 + "px";
              break;
          }
      ...
      close: function (target) {
        ...
        if (
          _config.modal.position.indexOf(cb.settings.effect) > -1 &&
          cb.settings.animation.length > 1
        ) {
          cb.modal.classList.remove(
            "custombox-modal-" +
              cb.settings.effect +
              "-" +
              cb.settings.animation[0]
          );
          cb.modal.classList.add(
            "custombox-modal-" +
              cb.settings.effect +
              "-" +
              cb.settings.animation[1].trim()
          );
        }
      ...
      responsive: function () {
        ...
        for (var i = 0, t = this.cb.length, result; i < t; i++) {
          ...
          // Top.
          if (
            this.cb[i].content.offsetHeight >= window.innerHeight &&
            this.cb[i].settings.width !== "full"
          ) {
            ...
            switch (this.cb[i].settings.position[1].trim()) {
              ...
              case "bottom":
                result =
                  window.innerHeight - this.cb[i].content.offsetHeight + "px";
                break;
              default:
                result =
                  window.innerHeight / 2 -
                  this.cb[i].content.offsetHeight / 2 +
                  "px";
                break;
            }
      ...
      binds: function () {
        ...
        cb.wrapper.event = function (event) {
          ...
          if (
            !stop &&
            event.target === cb.wrapper &&
            _this.get() &&
            _this.get().settings.overlayClose
          ) {
            stop = true;
            _this.close();
          }
        ...
        var callback = function () {
          ...
          if (!cb.inline) {
            for (
              var i = 0,
                script = cb.modal.getElementsByTagName("script"),
                t = script.length;
              i < t;
              i++
            ) {
