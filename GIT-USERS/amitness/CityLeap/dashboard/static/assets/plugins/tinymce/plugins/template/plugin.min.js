tinymce.PluginManager.add("template", function (a) {
  ...
  function c(b) {
        ...
        if (-1 == b.indexOf("<html>")) {
          ...
          tinymce.each(a.contentCSS, function (b) {
            c +=
              ...
              a.documentBaseURI.toAbsolute(b) +
              '">';
          }),
            (b =
    ...
    return b && 0 !== b.length
      ? (tinymce.each(b, function (a) {
        ...
        void d.find("listbox")[0].fire("select"))
      : void a.windowManager.alert("No templates defined");
  }
  function d(b, c) {
    function d(a, b) {
      if (((a = "" + a), a.length < b))
  ...
  function g(b, c) {
    ...
    (c = f(c, "template_replace_values")),
      ...
      j &&
        j.length > 0 &&
        ((i = k.create("div", null)), i.appendChild(j[0].cloneNode(!0))),
      h(k.select("*", i), function (b) {
        g(
          b,
          a.getParam("template_cdate_classes", "cdate").replace(/\s+/g, "|")
        ) &&
          (b.innerHTML = d(
            a.getParam(
              "template_cdate_format",
              a.getLang("template.cdate_format")
            )
          )),
          g(
            b,
            a.getParam("template_mdate_classes", "mdate").replace(/\s+/g, "|")
          ) &&
            (b.innerHTML = d(
              a.getParam(
                "template_mdate_format",
                a.getLang("template.mdate_format")
              )
            )),
          g(
            b,
  ...
  a.addCommand("mceInsertTemplate", g),
    a.addButton("template", { title: "Insert template", onclick: b(c) }),
    ...
    a.on("PreProcess", function (b) {
      ...
      h(c.select("div", b.node), function (b) {
        c.hasClass(b, "mceTmpl") &&
          (h(c.select("*", b), function (b) {
            c.hasClass(
              b,
              a.getParam("template_mdate_classes", "mdate").replace(/\s+/g, "|")
            ) &&
