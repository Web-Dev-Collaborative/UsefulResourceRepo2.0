(function (factory) {
  ...
  $.summernote.addPlugin({
    ...
    moveDown: function ($popover) {
      ...
      this.activate(
        $popover,
        index === -1 ? 0 : (index + 1) % $popover.children().length
      );
    },
    ...
    init: function (layoutInfo) {
      ...
      $note.on(
        "summernote.keydown",
        function HintKeyDown(customEvent, nativeEvent) {
      ...
      $note.on(
        "summernote.keyup",
        function HintKeyUp(customEvent, nativeEvent) {
          if (DROPDOWN_KEYCODES.indexOf(nativeEvent.keyCode) > -1) {
            ...
            timer = setTimeout(function () {
              ...
              self.searchKeyword(word.toString(), function (searchList) {
                ...
                $popover
                  .html(self.createTemplate(searchList))
                  .css({
                    left: rect.left,
    ...
    events: {
      ENTER: function (e, editor, layoutInfo) {
        if (
          layoutInfo.popover().find(".hint-group").css("display") !== "block"
        ) {
