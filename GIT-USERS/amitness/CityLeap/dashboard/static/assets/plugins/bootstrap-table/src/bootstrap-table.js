...
!(function ($) {
  ...
  var cellHeight = 37, // update css if changed
    cachedWidth = null,
    arrowAsc =
      "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABMAAAATCAYAAAByUDbMAAAAZ" +
    ...
    arrowBoth =
      "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABMAAAATCAQAAADYWf5HAAAAkElEQVQoz7X" +
    ...
    arrowDesc =
      "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABMAAAATCAYAAAByUDbMAAAAZUlEQVQ4y2NgGAWj" +
  ...
  BootstrapTable.prototype.initHeader = function () {
    ...
    $.each(this.options.columns, function (i, column) {
      ...
      style += sprintf(
        "width: %s%s; ",
        column.checkbox || column.radio ? 36 : width,
        unitWidth
      );
      ...
      html.push(
        sprintf(
          ...
          that.options.sortable && column.sortable ? "sortable" : ""
        )
      );
  ...
  BootstrapTable.prototype.initToolbar = function () {
    ...
    if (this.options.showPaginationSwitch) {
      html.push(
        sprintf(
          '<button class="btn btn-default" type="button" name="paginationSwitch" title="%s">',
    ...
    if (this.options.showRefresh) {
      this.$toolbar
        ...
        .off("click")
        .on("click", $.proxy(this.refresh, this));
    }
    ...
    if (this.options.showColumns) {
      ...
      $keepOpen
        ...
        .on("click", function () {
          ...
          that.toggleColumn(
            getFieldIndex(that.options.columns, $(this).data("field")),
            $this.prop("checked"),
            false
          );
          that.trigger(
              ...
              var value = item[key],
                column =
                  that.options.columns[
                    getFieldIndex(that.options.columns, key)
                  ],
  ...
  BootstrapTable.prototype.initPagination = function () {
    ...
    if (this.options.totalRows) {
      ...
      this.totalPages =
        ~~((this.options.totalRows - 1) / this.options.pageSize) + 1;
    ...
    if (typeof this.options.pageList === "string") {
      var list = this.options.pageList
        .replace("[", "")
        .replace("]", "")
        .replace(/ /g, "")
        .split(",");
      ...
      $.each(list, function (i, value) {
        pageList.push(
          value.toUpperCase() === that.options.formatAllRows().toUpperCase()
            ? that.options.formatAllRows()
            : +value
        );
      });
    }
    ...
    $.each(pageList, function (i, page) {
      if (
        ...
        if ($allSelected) {
          active =
            page === that.options.formatAllRows() ? ' class="active"' : "";
        } else {
    ...
    html.push(
      ...
      '<ul class="pagination' +
        ...
        '">',
      '<li class="page-first"><a href="javascript:void(0)">' +
        this.options.paginationFirstText +
        "</a></li>",
    ...
    html.push(
      '<li class="page-next"><a href="javascript:void(0)">' +
        ...
        "</a></li>",
      '<li class="page-last"><a href="javascript:void(0)">' +
        this.options.paginationLastText +
        "</a></li>",
      "</ul>",
      "</div>"
    );
  ...
  BootstrapTable.prototype.initBody = function (fixedScroll) {
    ...
    for (var i = this.pageFrom - 1; i < this.pageTo; i++) {
      ...
      attributes = calculateObjectValue(
        this.options,
      ...
      if (attributes) {
        for (key in attributes) {
          htmlAttributes.push(
            sprintf('%s="%s"', key, escapeHTML(attributes[key]))
          );
        }
      }
      ...
      html.push(
        ...
        sprintf(
          ' class="%s"',
          style.classes || ($.isArray(item) ? undefined : item._class)
        ),
      ...
      $.each(this.header.fields, function (j, field) {
        var text = "",
          ...
          rowspan_ = "",
          column =
            that.options.columns[getFieldIndex(that.options.columns, field)];
      ...
      .on("click", function () {
        ...
        if (that.options.clickToSelect) {
          if (that.header.clickToSelects[$tr.children().index($(this))]) {
            $tr
              .find(sprintf('[name="%s"]', that.options.selectItemName))[0]
              .click(); // #144: .trigger('click') bug
          }
        }
      });
    this.$body
      ...
      if ($(th).data("field") === that.options.sortName) {
        $(th)
          ...
          .css(
            "background-image",
            "url(" +
              (that.options.sortOrder === "desc" ? arrowDesc : arrowAsc) +
              ")"
          );
      } else {
        $(th)
          .find(".sortable")
          .css("background-image", "url(" + arrowBoth + ")");
      }
    });
  };
  ...
  BootstrapTable.prototype.updateRows = function () {
    ...
    this.$selectItem.each(function () {
      that.data[$(this).data("index")][that.header.stateField] =
        $(this).prop("checked");
    });
  };
  ...
  BootstrapTable.prototype.fitFooter = function () {
    ...
    this.$tableBody
      .find("tbody tr:first-child:not(.no-records-found) > td")
      .each(function (i) {
  ...
  BootstrapTable.prototype.toggleRow = function (index, isIdField, visible) {
    ...
    $(this.$body[0])
      .children()
      .filter(
        sprintf(isIdField ? '[data-uniqueid="%s"]' : '[data-index="%s"]', index)
      )
      [visible ? "show" : "hide"]();
  };
  ...
  BootstrapTable.prototype.removeByUniqueId = function (id) {
    ...
    for (i = len - 1; i >= 0; i--) {
      ...
      if (typeof row[uniqueId] === "string") {
        ...
        if (
          Number(row[uniqueId]) === row[uniqueId] &&
          row[uniqueId] % 1 === 0
        ) {
          id = parseInt(id);
        } else if (
          row[uniqueId] === Number(row[uniqueId]) &&
          row[uniqueId] !== 0
        ) {
          id = parseFloat(id);
        }
  ...
  BootstrapTable.prototype.showRow = function (params) {
    ...
    this.toggleRow(
      params.index,
      params.isIdField === undefined ? false : true,
      true
    );
  };
  ...
  BootstrapTable.prototype.updateCell = function (params) {
    if (
      !params.hasOwnProperty("rowIndex") ||
      !params.hasOwnProperty("fieldName") ||
      !params.hasOwnProperty("fieldValue")
    ) {
      return;
    }
