...
!(function ($) {
  ...
  var showAvdSearch = function (pColumns, searchTitle, searchText, that) {
    if (!$("#avdSearchModal").hasClass("modal")) {
      var vModal =
        '<div id="avdSearchModal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel" aria-hidden="true">';
      ...
      vModal +=
        '   <div class="container-fluid" id="avdSearchModalContent" style="padding-right: 0px;padding-left: 0px;" >';
      vModal += "   </div>";
      vModal += "  </div>";
      vModal += "  </div>";
      vModal += " </div>";
  ...
  var createFormAvd = function (pColumns, searchText, that) {
    ...
    htmlForm.push(
      sprintf(
        '<button type="button" id="btnCloseAvd" class="btn btn-default" >%s</button>',
        searchText
      )
    );
    htmlForm.push("</div>");
  ...
  BootstrapTable.prototype.initToolbar = function () {
    ...
    html.push(
      sprintf(
        ...
        that.options.iconSize === undefined
          ? ""
          : " btn-" + that.options.iconSize,
    ...
    that.$toolbar
      ...
      .on("click", function () {
        showAvdSearch(
          ...
          that.options.formatAdvancedSearch(),
          that.options.formatAdvancedCloseButton(),
          that
        );
      });
  };
  ...
  BootstrapTable.prototype.initSearch = function () {
    ...
    var that = this;
    var fp = $.isEmptyObject(this.filterColumnsPartial)
      ? null
    ...
    this.data = fp
      ? $.grep(this.data, function (item, i) {
          for (var key in fp) {
            ...
            if (
              !(
                $.inArray(key, that.header.fields) !== -1 &&
