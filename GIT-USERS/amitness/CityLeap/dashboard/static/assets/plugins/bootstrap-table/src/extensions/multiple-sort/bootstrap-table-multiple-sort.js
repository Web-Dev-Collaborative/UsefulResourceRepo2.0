...
(function ($) {
  ...
  var showSortModal = function (that) {
    if (!$("#sortModal").hasClass("modal")) {
      ...
      sModal +=
        '                     <h4 class="modal-title" id="sortModalLabel">' +
      ...
      sModal +=
        '                                     <button id="add" type="button" class="btn btn-default"><i class="' +
        that.options.iconsPrefix +
        " " +
        that.options.icons.plus +
        '"></i> ' +
        that.options.formatAddLevel() +
        "</button>";
      sModal +=
        '                                     <button id="delete" type="button" class="btn btn-default" disabled><i class="' +
        that.options.iconsPrefix +
        " " +
        that.options.icons.minus +
        '"></i> ' +
          ...
          if (results.length > 0) {
            if ($alert.length === 0) {
              $alert =
                ...
                that.options.formatDuplicateAlertTitle() +
                "</strong> " +
                that.options.formatDuplicateAlertDescription() +
                "</div>";
      ...
      if (that.options.sortPriority !== null) {
        if (
          $rows.length < that.options.sortPriority.length &&
  ...
  BootstrapTable.prototype.initToolbar = function () {
    ...
    if (this.options.showMultiSort) {
      ...
      if (!$multiSortBtn.length) {
        $multiSortBtn =
          '  <button class="multi-sort btn btn-default' +
          (this.options.iconSize === undefined
            ? ""
            : " btn-" + this.options.iconSize) +
          '" type="button" data-toggle="modal" data-target="#sortModal" title="' +
          this.options.formatMultipleSort() +
          '">';
        $multiSortBtn +=
          '     <i class="' +
          this.options.iconsPrefix +
          " " +
          this.options.icons.sort +
          '"></i>';
        $multiSortBtn += "</button>";
  ...
  BootstrapTable.prototype.addLevel = function (index, sortPriority) {
    var $sortModal = $("#sortModal"),
      text =
        index === 0 ? this.options.formatSortBy() : this.options.formatThenBy();

    $sortModal.find("tbody").append(
      $("<tr>")
        .append($("<td>").text(text))
        .append(
          $("<td>").append($('<select class="form-control multi-sort-name">'))
        )
        .append(
          $("<td>").append($('<select class="form-control multi-sort-order">'))
        )
    );
    ...
    if (sortPriority !== undefined) {
      $multiSortName
        .find('option[value="' + sortPriority.sortName + '"]')
        .attr("selected", true);
      $multiSortOrder
        .find('option[value="' + sortPriority.sortOrder + '"]')
        .attr("selected", true);
    }
  };
  ...
  BootstrapTable.prototype.assignSortableArrows = function () {
    ...
    for (var i = 0; i < headers.length; i++) {
      for (var c = 0; c < that.options.sortPriority.length; c++) {
        if (
          $(headers[i]).data("field") === that.options.sortPriority[c].sortName
        ) {
          $(headers[i])
            .find(".sortable")
            .css(
              "background-image",
              "url(" +
                (that.options.sortPriority[c].sortOrder === "desc"
                  ? arrowDesc
                  : arrowAsc) +
                ")"
            );
