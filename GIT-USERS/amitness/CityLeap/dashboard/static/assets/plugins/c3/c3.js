(function (window) {
  ...
  c3_chart_internal_fn.initParams = function () {
    ...
    $$.dataTimeFormat = config.data_xLocaltime
      ? d3.time.format
      : d3.time.format.utc;
    $$.axisTimeFormat = config.axis_x_localtime
      ? d3.time.format
      : d3.time.format.utc;
  ...
  c3_chart_internal_fn.getScale = function (min, max, forTimeseries) {
    return (
      forTimeseries ? this.d3.time.scale() : this.d3.scale.linear()
    ).range([min, max]);
  };
  ...
  c3_chart_internal_fn.updateXDomain = function (
    targets,
    withUpdateXDomain,
    withUpdateOrgXDomain,
    withTrim,
  ...
  c3_chart_internal_fn.convertUrlToData = function (url, mimeType, keys, done) {
    ...
    $$.d3.xhr(url, function (error, data) {
      ...
      if (!data) {
        throw new Error(
          error.responseURL + " " + error.status + " (" + error.statusText + ")"
        );
      }
      if (type === "json") {
  ...
  c3_chart_internal_fn.generateEventRectsForSingleX = function (
    eventRectEnter
  ) {
    var $$ = this,
      d3 = $$.d3,
    ...
    eventRectEnter
      ...
      .on("mousemove", function (d) {
        ...
        // Show tooltip
        selectedData = $$.filterTargetsToShow($$.data.targets).map(function (
          t
        ) {
        ...
        $$.main
          ...
          .each(function () {
            ...
            if (config.data_selection_enabled) {
              eventRect.style(
                "cursor",
                config.data_selection_grouped ? "pointer" : null
              );
            }
            if (!config.tooltip_grouped) {
              $$.hideXGridFocus();
  ...
  c3_chart_internal_fn.generateEventRectsForMultipleXs = function (
    eventRectEnter
  ) {
    var $$ = this,
      d3 = $$.d3,
  ...
  c3_chart_internal_fn.getShapeX = function (
    offset,
    targetsNum,
    indices,
    isSub
  ...
  c3_chart_internal_fn.generateDrawArea = function (areaIndices, isSub) {
    ...
    area = config.axis_rotated
      ...
      : area.x(xValue).y0(value0).y1(value1);
    if (!config.line_connectNull) {
  ...
  c3_chart_internal_fn.redrawText = function (
    xForText,
    yForText,
    forFlow,
    withTransition
  ...
  c3_chart_internal_fn.generateXYForText = function (
    areaIndices,
    barIndices,
    lineIndices,
    forX
  ...
  c3_chart_internal_fn.updateXGrid = function (withoutUpdate) {
    ...
    if (!withoutUpdate) {
      $$.xgrid.attr($$.xgridAttr).style("opacity", function () {
        return +d3.select(this).attr(config.axis_rotated ? "y1" : "x1") ===
          (config.axis_rotated ? $$.height : 0)
          ? 0
          : 1;
      });
    }
  ...
  c3_chart_internal_fn.getTooltipContent = function (
    d,
    defaultTitleFormat,
    defaultValueFormat,
    color
  ...
  c3_chart_internal_fn.tooltipPosition = function (
    dataToShow,
    tWidth,
    tHeight,
    element
  ...
  c3_chart_internal_fn.showTooltip = function (selectedData, element) {
    ...
    var forArc = $$.hasArcType(),
      ...
      positionFunction =
        config.tooltip_position || c3_chart_internal_fn.tooltipPosition;
  ...
  c3_chart_internal_fn.updateSizeForLegend = function (
    legendHeight,
    legendWidth
  ) {
    var $$ = this,
  ...
  c3_chart_internal_fn.updateLegend = function (
    targetIds,
    options,
    transitions
  ) {
  ...
  Axis.prototype.getYAxis = function getYAxis(
    ...
    if ($$.isTimeSeriesY()) {
      axis.ticks(
        d3.time[config.axis_y_tick_time_value],
  ...
  c3_chart_internal_fn.redrawArc = function (
    duration,
    durationForExit,
    withTransform
  ) {
    ...
    if ($$.hasType("gauge")) {
      ...
      $$.arcs
        ...
        .attr(
          "dx",
          -1 * ($$.innerRadius + ($$.radius - $$.innerRadius) / 2) + "px"
        )
        .attr("dy", "1.2em")
  ...
  c3_chart_internal_fn.redrawBarForSubchart = function (
    drawBarOnSub,
    withTransition,
    duration
  ) {
    (withTransition
      ? this.contextBar.transition().duration(duration)
      : this.contextBar
    )
      .attr("d", drawBarOnSub)
      .style("opacity", 1);
  ...
  c3_chart_internal_fn.redrawLineForSubchart = function (
    drawLineOnSub,
    withTransition,
    duration
  ) {
    (withTransition
      ? this.contextLine.transition().duration(duration)
      : this.contextLine
    )
  ...
  c3_chart_internal_fn.redrawAreaForSubchart = function (
    drawAreaOnSub,
    withTransition,
    duration
  ) {
    (withTransition
      ? this.contextArea.transition().duration(duration)
      : this.contextArea
    )
  ...
  c3_chart_internal_fn.transformContext = function (
    withTransition,
    transitions
  ) {
    var $$ = this,
  ...
  c3_chart_internal_fn.initZoom = function () {
    ...
    $$.zoom = d3.behavior
      ...
      .on("zoomstart", function () {
        ...
        $$.zoom.altDomain = d3.event.sourceEvent.altKey
          ? $$.x.orgDomain()
          : null;
      ...
      .on("zoomend", function () {
        ...
        if (
          event &&
          startEvent.clientX === event.clientX &&
          startEvent.clientY === event.clientY
        ) {
          return;
        }
        $$.redrawEventRect();
  ...
  var isValue = (c3_chart_internal_fn.isValue = function (v) {
    ...
    getOption = (c3_chart_internal_fn.getOption = function (
      options,
      key,
      defaultValue
    ) {
  ...
  c3_chart_internal_fn.transformTo = function (
    targetIds,
    type,
    optionsForRedraw
  ) {
  ...
  c3_chart_fn.tooltip.show = function (args) {
    ...
    if (args.data) {
      if ($$.isMultipleX()) {
        ...
        index = isValue(args.data.index)
          ? args.data.index
          : $$.getIndexByX(args.data.x);
      }
  ...
  function c3_axis(d3, params) {
    ...
    function axis(g) {
      g.each(function () {
        ...
        var range = scale.rangeExtent
            ? scale.rangeExtent()
            : scaleExtent(scale.range()),
        ...
        function splitTickText(d, maxWidth) {
          ...
          if (!maxWidth || maxWidth <= 0) {
            maxWidth = isVertical
              ? 95
              : params.isCategory
              ? Math.ceil(scale1(ticks[1]) - scale1(ticks[0])) - 12
              : 110;
          }
