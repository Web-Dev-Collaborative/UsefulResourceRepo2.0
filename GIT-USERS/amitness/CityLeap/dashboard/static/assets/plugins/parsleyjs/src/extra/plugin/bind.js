  ...
  window.ParsleyExtend = $.extend(window.ParsleyExtend, {
    ...
    _bindFields: function () {
      ...
      if ("undefined" === typeof this.options.fields)
        throw new Error(
          "bind.js plugin needs to have Parsley instantiated with fields"
        );
      ...
      for (var selector in this.options.fields) {
        ...
        for (var name in this.options.fields[selector]) {
          if (
            "object" === typeof this.options.fields[selector][name] &&
            !(this.options.fields[selector][name] instanceof Array)
          )
            field.addConstraint(
              name.toLowerCase(),
              this.options.fields[selector][name].requirements,
              this.options.fields[selector][name].priority || 32
            );
          else
            field.addConstraint(
              name.toLowerCase(),
              this.options.fields[selector][name]
            );
        }
      }
