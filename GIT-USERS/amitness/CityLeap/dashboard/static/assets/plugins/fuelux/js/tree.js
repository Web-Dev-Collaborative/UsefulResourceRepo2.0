...
(function (factory) {
  ...
  Tree.prototype = {
      ...
      this.options.dataSource(treeData ? treeData : {}, function (items) {
        ...
        $.each(items.data, function (index, value) {
          ...
          if (value.type === "folder") {
            $entity = self.$element
              .find("[data-template=treebranch]:eq(0)")
              .clone()
              .removeClass("hide hidden")
              .removeData("template"); // hide is deprecated
            $entity.data(value);
            $entity
              .find(".tree-branch-name > .tree-label")
              .html(value.text || value.name);
          } else if (value.type === "item") {
            $entity = self.$element
              .find("[data-template=treeitem]:eq(0)")
              .clone()
              .removeClass("hide hidden")
              .removeData("template"); // hide is deprecated
            $entity
              .find(".tree-item-name > .tree-label")
              .html(value.text || value.name);
            $entity.data(value);
          }
    ...
    discloseFolder: function discloseFolder(el) {
      ...
      $branch
        .find("> .tree-branch-header .icon-folder")
        .eq(0)
    ...
    collapse: function collapse() {
      ...
      var closedReported = function closedReported(event, closed) {
        ...
        // hide is deprecated
        if (
          self.$element.find(".tree-branch.tree-open:not('.hidden, .hide')")
            .length === 0
        ) {
      ...
      self.$element
        .find(".tree-branch.tree-open:not('.hidden, .hide')")
        .each(function () {
    ...
    discloseVisible: function discloseVisible() {
      ...
      var $openableFolders = self.$element.find(
        ".tree-branch:not('.tree-open, .hidden, .hide')"
      );
      var reportedOpened = [];
      ...
      self.$element
        .find(".tree-branch:not('.tree-open, .hidden, .hide')")
        .each(function triggerOpen() {
    ...
    discloseAll: function discloseAll() {
      ...
      var isExceededLimit =
        self.options.disclosuresUpperLimit >= 1 &&
        self.$element.data("disclosures") >= self.options.disclosuresUpperLimit;
      var isAllDisclosed =
        self.$element.find(".tree-branch:not('.tree-open, .hidden, .hide')")
          .length === 0;
      ...
      if (!isAllDisclosed) {
        ...
        self.$element.data(
          "disclosures",
          self.$element.data("disclosures") + 1
        );
  ...
  function styleNodeSelected($element, $icon) {
    ...
    if (
      ...
      $icon.hasClass("fueluxicon-bullet")
    ) {
