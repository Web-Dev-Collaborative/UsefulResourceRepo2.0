...
(function (factory) {
  ...
  if (
    ...
    throw new Error(
      "Fuel UX scheduler control requires combobox, datepicker, radio, selectlist, and spinbox."
    );
  }
  ...
  var Scheduler = function (element, options) {
    ...
    this.$repeatIntervalSelect.on(
      "changed.fu.selectlist",
      $.proxy(this.repeatIntervalSelectChanged, this)
    );
    this.$endSelect.on(
      "changed.fu.selectlist",
      $.proxy(this.endSelectChanged, this)
    );
    this.$element
      .find(".repeat-days-of-the-week .btn-group .btn")
      .on("change.fu.scheduler", function (e, data) {
        self.changed(e, data, true);
      });
    this.$element
      .find(".combobox")
      .on("changed.fu.combobox", $.proxy(this.changed, this));
    this.$element
      .find(".datepicker")
      .on("changed.fu.datepicker", $.proxy(this.changed, this));
    this.$element
      .find(".datepicker")
      .on("dateClicked.fu.datepicker", $.proxy(this.changed, this));
    this.$element
      .find(".selectlist")
      .on("changed.fu.selectlist", $.proxy(this.changed, this));
    this.$element
      .find(".spinbox")
      .on("changed.fu.spinbox", $.proxy(this.changed, this));
    this.$element
      .find(".repeat-monthly .radio-custom, .repeat-yearly .radio-custom")
      .on("change.fu.scheduler", $.proxy(this.changed, this));
  };
  ...
  Scheduler.prototype = {
    ...
    setUtcTime: function (d, t, offset) {
      ...
      if (offset === "Z") {
        ...
        if (m !== null) {
          ...
          utcDate.setUTCHours(
            utcDate.getUTCHours() + modifier * parseInt(d1, 10)
          );
        }
      }
    ...
    getValue: function () {
      ...
      startDateTime =
        "" + getFormattedDate(this.$startDate.datepicker("getDate"), "-");
      ...
      if (repeat === "none") {
        ...
        days = [];
        this.$element
          .find(".repeat-days-of-the-week .btn-group input:checked")
          .each(function () {
        ...
        if (type === "bymonthday") {
          day = parseInt(
            this.$element
              .find(".repeat-monthly-date .selectlist")
              .selectlist("selectedItem").text,
            10
          );
          pattern += "BYMONTHDAY=" + day + ";";
        } else if (type === "bysetpos") {
          days = this.$element
            .find(".repeat-monthly-day .month-days")
            .selectlist("selectedItem").value;
          pos = this.$element
            .find(".repeat-monthly-day .month-day-pos")
            .selectlist("selectedItem").value;
          pattern += "BYDAY=" + days + ";";
        ...
        if (type === "bymonthday") {
          ...
          month = this.$element
            .find(".repeat-yearly-date .year-month")
            .selectlist("selectedItem").value;
          day = this.$element
            .find(".repeat-yearly-date .year-month-day")
            .selectlist("selectedItem").text;
          pattern += "BYMONTH=" + month + ";";
          pattern += "BYMONTHDAY=" + day + ";";
        } else if (type === "bysetpos") {
          days = this.$element
            .find(".repeat-yearly-day .year-month-days")
            .selectlist("selectedItem").value;
          pos = this.$element
            .find(".repeat-yearly-day .year-month-day-pos")
            .selectlist("selectedItem").value;
          // there are multiple .year-month classed elements in scheduler markup
          month = this.$element
            .find(".repeat-yearly-day .year-month")
            .selectlist("selectedItem").value;
      ...
      if (repeat !== "none") {
        if (end === "after") {
          ...
          duration =
            "UNTIL=" +
            getFormattedDate(this.$endDate.datepicker("getDate"), "") +
            ";";
        }
      }
    ...
    setValue: function (options) {
      ...
      if (options.startDateTime) {
        ...
        if (temp[1]) {
          ...
          minutes = temp[1][1]
            ? parseInt(temp[1][1].split("+")[0].split("-")[0].split("Z")[0], 10)
            : 0;
      ...
      if (options.recurrencePattern) {
        ...
        if (recur.FREQ === "DAILY") {
          ...
          this.$element
            .find(".repeat-monthly input")
            .removeAttr("checked")
            .removeClass("checked");
          this.$element
            .find(".repeat-monthly label.radio-custom")
            .removeClass("checked");
          if (recur.BYMONTHDAY) {
            ...
            temp
              .find(".selectlist")
              .selectlist("selectByValue", recur.BYMONTHDAY);
          } else if (recur.BYDAY) {
            ...
            if (recur.BYSETPOS) {
              temp
                .find(".month-day-pos")
                .selectlist("selectByValue", recur.BYSETPOS);
            }
          ...
          this.$element
            .find(".repeat-yearly input")
            .removeAttr("checked")
            .removeClass("checked");
          this.$element
            .find(".repeat-yearly label.radio-custom")
            .removeClass("checked");
          if (recur.BYMONTHDAY) {
            ...
            if (recur.BYMONTH) {
              temp
                .find(".year-month")
                .selectlist("selectByValue", recur.BYMONTH);
            }
            ...
            temp.find("label.radio-custom").addClass("checked");
            temp
              .find(".year-month-day-pos")
              .selectlist("selectByValue", recur.BYSETPOS);
            if (recur.BYDAY) {
              temp
                .find(".year-month-days")
                .selectlist("selectByValue", recur.BYDAY);
            }
        ...
        if (recur.COUNT) {
          ...
          var timeZone = this.$timeZone.selectlist("selectedItem");
          var timezoneOffset =
            timeZone.offset === "+00:00" ? "Z" : timeZone.offset;
    ...
    toggleState: function (action) {
      ...
      this.$element
        .find(".repeat-days-of-the-week .btn-group")
        [action]("disabled");
    },
  ...
  $(document).on(
    "mousedown.fu.scheduler.data-api",
    "[data-initialize=scheduler]",
    function (e) {
