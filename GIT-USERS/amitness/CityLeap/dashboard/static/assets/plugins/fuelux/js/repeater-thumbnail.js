...
(function (factory) {
  ...
  if ($.fn.repeater) {
    //ADDITIONAL METHODS
    $.fn.repeater.Constructor.prototype.thumbnail_clearSelectedItems =
      function () {
        this.$canvas
          .find(".repeater-thumbnail-cont .selectable.selected")
          .removeClass("selected");
      };

    $.fn.repeater.Constructor.prototype.thumbnail_getSelectedItems =
      function () {
        var selected = [];
        this.$canvas
          .find(".repeater-thumbnail-cont .selectable.selected")
    ...
    $.fn.repeater.Constructor.prototype.thumbnail_setSelectedItems = function (
      items,
      force
    ) {
      ...
      for (i = 0; i < l; i++) {
        if (items[i].index !== undefined) {
          ...
          n = 0;
          this.$canvas
            .find(".repeater-thumbnail-cont .selectable")
            .each(compareItemIndex);
          if ($item.length > 0) {
            selectItem($item, items[i].selected);
          }
    ...
    $.fn.repeater.viewTypes.thumbnail = {
      ...
      before: function (helpers) {
        ...
        if ($cont.length < 1) {
          $cont = $(
            '<div class="clearfix repeater-thumbnail-cont" data-container="true" data-infinite="true" data-preserve="shallow"></div>'
          );
      ...
      renderItem: function (helpers) {
        ...
        var $thumbnail = $(
          fillTemplate(
            ...
            this.viewOptions.thumbnail_template
          )
        );
  ...
  function fillTemplate(itemData, template) {
    ...
    function replace() {
      ...
      if (start > -1 && end > -1) {
        ...
        template =
          template.substring(0, start) + val + template.substring(end + 2);
      } else {
        invalid = true;
      }
    }
