...
(function (factory) {
  ...
  var MOMENT_NOT_AVAILABLE =
    "moment.js is not available so you cannot use this function";
  ...
  var Datepicker = function (element, options) {
    ...
    this.$calendar
      .find(".datepicker-today")
      .on("click.fu.datepicker", $.proxy(this.todayClicked, this));
    this.$days.on(
      "click.fu.datepicker",
      "tr td button",
      $.proxy(this.dateClicked, this)
    );
    this.$header
      .find(".next")
      .on("click.fu.datepicker", $.proxy(this.next, this));
    this.$header
      .find(".prev")
      .on("click.fu.datepicker", $.proxy(this.prev, this));
    this.$headerTitle.on(
      "click.fu.datepicker",
      $.proxy(this.titleClicked, this)
    );
    ...
    this.$wheels
      .find(".datepicker-wheels-back")
      .on("click.fu.datepicker", $.proxy(this.backClicked, this));
    this.$wheels
      .find(".datepicker-wheels-select")
      .on("click.fu.datepicker", $.proxy(this.selectClicked, this));
    this.$wheelsMonth.on(
      "click.fu.datepicker",
      "ul button",
      $.proxy(this.monthClicked, this)
    );
    this.$wheelsYear.on(
      "click.fu.datepicker",
      "ul button",
      $.proxy(this.yearClicked, this)
    );
    this.$wheelsYear
      .find("ul")
      .on("scroll.fu.datepicker", $.proxy(this.onYearScroll, this));
    ...
    var init = function () {
      ...
      if (this.sameYearOnly) {
        this.yearRestriction = this.selectedDate
  ...
  Datepicker.prototype = {
    ...
    checkForMomentJS: function () {
      if (
        ($.isFunction(window.moment) ||
          (typeof moment !== "undefined" && $.isFunction(moment))) &&
        $.isPlainObject(this.options.momentConfig) &&
        typeof this.options.momentConfig.culture === "string" &&
        typeof this.options.momentConfig.format === "string"
      ) {
        return true;
      } else {
        return false;
      ...
      for (i = 0, l = restricted.length; i < l; i++) {
        ...
        if (
          ...
          (year < to.year ||
            ...
            (year === to.year && month === to.month && date <= to.date))
        ) {
          return true;
        }
      }
    ...
    onYearScroll: function (e) {
      ...
      var height =
        $yearUl.css("box-sizing") === "border-box"
          ? $yearUl.outerHeight()
          : $yearUl.height();
      ...
      if (topPercentage < 5) {
        ...
        $yearUl.scrollTop(
          $yearUl.get(0).scrollHeight - scrollHeight + scrollTop
        );
        this.artificialScrolling = false;
    ...
    renderMonth: function (date) {
      ...
      for (i = 0; i < rows; i++) {
        ...
        for (j = 0; j < 7; j++) {
          ...
          if ($td.hasClass("restricted")) {
            $td.html(
              '<span><b class="datepicker-date">' + curDate + "</b></span>"
            );
          } else {
            $td.html(
              '<span><button type="button" class="datepicker-date">' +
                curDate +
                "</button></span>"
            );
          }
    ...
    renderWheel: function (date) {
      ...
      $monthUl.scrollTop(
        $monthUl.scrollTop() +
          ($monthSelected.position().top -
            $monthUl.outerHeight() / 2 -
            $monthSelected.outerHeight(true) / 2)
      );
      ...
      $yearUl.scrollTop(
        $yearUl.scrollTop() +
          ($yearSelected.position().top -
            $yearUl.outerHeight() / 2 -
            $yearSelected.outerHeight(true) / 2)
      );
      this.artificialScrolling = false;
    ...
    titleClicked: function (e) {
      this.changeView(
        "wheels",
        new Date(
          this.$headerTitle.attr("data-year"),
          this.$headerTitle.attr("data-month"),
          1
        )
      );
    },
    ...
    todayClicked: function (e) {
      ...
      if (
        date.getMonth() + "" !== this.$headerTitle.attr("data-month") ||
        date.getFullYear() + "" !== this.$headerTitle.attr("data-year")
      ) {
        this.renderMonth(date);
      }
    },
  ...
  $(document).on(
    "mousedown.fu.datepicker.data-api",
  ...
  $(document).on(
    "click.fu.datepicker.data-api",
    ".datepicker .dropdown-menu",
    function (e) {
      var $target = $(e.target);
      if (
        !$target.is(".datepicker-date") ||
        $target.closest(".restricted").length
      ) {
        e.stopPropagation();
      }
    }
