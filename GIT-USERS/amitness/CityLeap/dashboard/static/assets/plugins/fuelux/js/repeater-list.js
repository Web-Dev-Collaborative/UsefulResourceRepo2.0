...
(function (factory) {
  ...
  if ($.fn.repeater) {
    ...
    $.fn.repeater.Constructor.prototype.list_clearSelectedItems = function () {
      ...
      this.$canvas
        .find(".repeater-list table tbody tr.selected")
        .removeClass("selected");
    };

    $.fn.repeater.Constructor.prototype.list_highlightColumn = function (
      index,
      force
    ) {
    ...
    $.fn.repeater.Constructor.prototype.list_getSelectedItems = function () {
      var selected = [];
      this.$canvas
        .find(".repeater-list .repeater-list-wrapper > table tbody tr.selected")
        .each(function () {
          var $item = $(this);
          selected.push({
    ...
    $.fn.repeater.Constructor.prototype.list_positionHeadings = function () {
      ...
      if (scrollLeft > 0) {
        $wrapper.find(".repeater-list-heading").each(function () {
          var $heading = $(this);
          var left =
            $heading.parents("th:first").offset().left - offsetLeft + "px";
    ...
    $.fn.repeater.Constructor.prototype.list_setSelectedItems = function (
      items,
      force
    ) {
        ...
        if (select) {
          ...
          if (!$itm.hasClass("selected")) {
            ...
            $itm
              .find("td:first")
              .prepend(
                '<div class="repeater-list-check"><span class="glyphicon glyphicon-ok"></span></div>'
              );
          }
        } else {
      ...
      for (i = 0; i < l; i++) {
        if (items[i].index !== undefined) {
          $item = this.$canvas.find(
            ".repeater-list table tbody tr:nth-child(" +
              (items[i].index + 1) +
              ")"
          );
          if ($item.length > 0) {
          ...
          items[i].property !== undefined &&
          items[i].value !== undefined
        ) {
          this.$canvas
            .find(".repeater-list table tbody tr")
            .each(checkIfItemMatchesValue);
        }
      }
    };
    ...
    $.fn.repeater.Constructor.prototype.list_setFrozenColumns = function () {
      ...
      var $table = this.$element.find(
        ".repeater-list .repeater-list-wrapper > table"
      );
      ...
      if (frozenTable.length < 1) {
        ...
        var $frozenColumnWrapper = $(
          '<div class="frozen-column-wrapper"></div>'
        ).insertBefore($table);
        ...
        var $frozenTheadWrapper = $(
          '<div class="frozen-thead-wrapper"></div>'
        ).append($frozenThead);
      ...
      this.$element
        .find(".repeater-list table.table-frozen tr")
      ...
      this.$element
        .find(".frozen-column-wrapper, .frozen-thead-wrapper")
        .width(columnWidth);

      $(".frozen-thead-wrapper .repeater-list-heading").on(
        "click",
        function () {
          ...
          self.$element
            .find(
              ".repeater-list-wrapper > table thead th:nth-child(" +
                index +
    ...
    $.fn.repeater.Constructor.prototype.list_positionColumns = function () {
      ...
      var scrollLeft = $wrapper.scrollLeft();
      var frozenEnabled =
        this.viewOptions.list_frozenColumns ||
        this.viewOptions.list_selectable === "multi";
      var actionsEnabled = this.viewOptions.list_actions;

      var canvasWidth = this.$element.find(".repeater-canvas").outerWidth();
      var tableWidth = this.$element
        .find(".repeater-list .repeater-list-wrapper > table")
        .outerWidth();

      var actionsWidth = this.$element.find(".table-actions")
        ? this.$element.find(".table-actions").outerWidth()
        : 0;

      var shouldScroll =
        tableWidth - (canvasWidth - actionsWidth) >= scrollLeft;
    ...
    $.fn.repeater.Constructor.prototype.list_createItemActions = function () {
      ...
      if ($actionsTable.length < 1) {
        var $actionsColumnWrapper = $(
          '<div class="actions-column-wrapper" style="width: ' +
            this.list_actions_width +
            'px"></div>'
        ).insertBefore($table);
        ...
        if (this.viewOptions.list_selectable === "multi") {
          $actionsColumn
            .find("thead tr")
            .html(
              '<th><div class="repeater-list-heading">' +
                selectlist +
                "</div></th>"
            );
          ...
          var label =
            this.viewOptions.list_actions.label ||
            '<span class="actions-hidden">a</span>';
          $actionsColumn
            .find("thead tr")
            .addClass("empty-heading")
            .html(
              "<th>" +
                label +
      ...
      this.$element
        .find(".repeater-list table.table-actions thead tr th")
        .outerHeight($table.find("thead tr th").outerHeight());
      this.$element
        .find(".repeater-list table.table-actions tbody tr td:first-child")
        .each(function (i, elem) {
          $(this).outerHeight(
            $table.find("tbody tr:eq(" + i + ") td").outerHeight()
          );
        });

      //row level actions click
      this.$element
        .find(".table-actions tbody .action-item")
        .on("click", function () {
      ...
      // bulk actions click
      this.$element
        .find(".table-actions thead .action-item")
        .on("click", function () {
          var actionName = $(this).data("action");
          var selected = {
            actionName: actionName,
          ...
          self.$element
            .find(".repeater-list-wrapper > table .selected")
            .each(function () {
    ...
    $.fn.repeater.Constructor.prototype.list_getActionItems = function (
      selected
    ) {
      var i;
      var selectedObj = [];
      ...
      for (i = 0; i < selected.rows.length; i++) {
        var clickedRow = this.$canvas.find(
          ".repeater-list-wrapper > table tbody tr:nth-child(" +
            selected.rows[i] +
            ")"
        );
        selectedObj.push({
    ...
    $.fn.repeater.Constructor.prototype.list_sizeActionsTable = function () {
      ...
      var $actionsTableHeading = this.$element.find(
        ".repeater-list-wrapper .actions-column-wrapper thead th .repeater-list-heading"
      );
      $actionsTableHeading.outerHeight(
        $table.find("thead th .repeater-list-heading").outerHeight()
      );
    };

    $.fn.repeater.Constructor.prototype.list_frozenOptionsInitialize =
      function () {
        var self = this;
        ...
        var $checkboxes = this.$element.find(
          ".frozen-column-wrapper .checkbox-inline"
        );
        ...
        this.$element
          ...
          .on("change", function (e) {
            ...
            self.$element
              .find(
                ".repeater-list-wrapper > table tbody tr:nth-child(" + row + ")"
              )
              .click();
          });

        this.$element
          .find(".frozen-thead-wrapper thead .checkbox-inline")
          .on("change", function () {
            if ($(this).checkbox("isChecked")) {
              self.$element
                .find(".repeater-list-wrapper > table tbody tr:not(.selected)")
                .click();
    ...
    $.fn.repeater.viewTypes.list = {
      ...
      initialize: function (helpers, callback) {
        ...
        this.list_actions_width =
          this.viewOptions.list_actions.width !== undefined
            ? this.viewOptions.list_actions.width
            : 37;
      ...
      before: function (helpers) {
        ...
        if ($listContainer.length < 1) {
          $listContainer = $(
            '<div class="repeater-list ' +
              specialBrowserClass() +
              '" data-preserve="shallow"><div class="repeater-list-wrapper" data-infinite="true" data-preserve="shallow"><table aria-readonly="true" class="table" data-preserve="shallow" role="grid"></table></div></div>'
          );
          $listContainer
            .find(".repeater-list-wrapper")
            .on("scroll.fu.repeaterList", function () {
              if (self.viewOptions.list_columnSyncing) {
                self.list_positionHeadings();
              }
            });
          if (
            self.viewOptions.list_frozenColumns ||
            self.viewOptions.list_actions ||
            self.viewOptions.list_selectable === "multi"
          ) {
        ...
        helpers.container.removeClass(
          "actions-enabled actions-enabled multi-select-enabled"
        );
      ...
      after: function () {
        ...
        $sorted = this.$canvas.find(
          ".repeater-list-wrapper > table .repeater-list-heading.sorted"
        );
        if ($sorted.length > 0) {
  ...
  function renderColumn($row, rows, rowIndex, columns, columnIndex) {
    ...
    if (this.viewOptions.list_actions !== false && property === "@_ACTIONS_@") {
      content =
        '<div class="repeater-list-actions-placeholder" style="width: ' +
        this.list_actions_width +
        'px"></div>';
    }
    ...
    if (
      this.viewOptions.list_selectable === "multi" &&
      columns[columnIndex].property === "@_CHECKBOX_@"
    ) {
      var checkBoxMarkup =
        '<label data-row="' +
        rowIndex +
        '" class="checkbox-custom checkbox-inline body-checkbox">' +
  ...
  function renderHeader($tr, columns, index) {
    ...
    var $div = $(
      '<div class="repeater-list-heading"><span class="glyphicon rlc"></span></div>'
    );
    var checkBoxMarkup =
      '<div class="repeater-list-heading header-checkbox"><label class="checkbox-custom checkbox-inline">' +
    ...
    if (
      this.viewOptions.list_actions &&
      columns[index].property === "@_ACTIONS_@"
    ) {
    ...
    if (sortable) {
      ...
      $div.on("click.fu.repeaterList", function () {
        self.list_sortProperty =
          typeof sortable === "string" ? sortable : columns[index].property;
    ...
    if (
      columns[index].sortDirection === "asc" ||
      columns[index].sortDirection === "desc"
    ) {
      $tr.find("th, .repeater-list-heading").removeClass("sorted");
  ...
  function renderRow($tbody, rows, index) {
    ...
    if (this.viewOptions.list_selectable) {
      ...
      $row.on("click.fu.repeaterList", function () {
        ...
        var index = $(this).index();
        index = index + 1;
        var $frozenRow = self.$element.find(
          ".frozen-column-wrapper tr:nth-child(" + index + ")"
        );
        var $actionsRow = self.$element.find(
          ".actions-column-wrapper tr:nth-child(" + index + ")"
        );
        var $checkBox = self.$element.find(
          ".frozen-column-wrapper tr:nth-child(" + index + ") .checkbox-inline"
        );
        ...
        if ($item.is(".selected")) {
          ...
          if (!isMulti) {
            ...
            self.$canvas
              .find(".repeater-list tbody tr.selected")
              .each(function () {
  ...
  function renderTbody($table, data) {
    ...
    if (typeof data.error === "string" && data.error.length > 0) {
      $empty = $(
        '<tr class="empty text-danger"><td colspan="' +
          this.list_columns.length +
          '"></td></tr>'
      );
  ...
  function renderThead($table, data) {
    ...
    if (
      this.list_firstRender ||
      differentColumns(this.list_columns, columns) ||
      $thead.length === 0
    ) {
      $thead.remove();
      ...
      if (this.viewOptions.list_selectable === "multi" && !this.list_noItems) {
        ...
        //its css class
        var checkboxWidth = this.$element
          .find(".repeater-list-wrapper .header-checkbox")
          .outerWidth();
