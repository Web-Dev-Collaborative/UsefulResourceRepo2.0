...
(function (factory) {
  ...
  var Pillbox = function (element, options) {
    ...
    this.$element.on(
      "click.fu.pillbox",
      ".pill-group > .pill",
      $.proxy(this.itemClicked, this)
    );
    this.$element.on("click.fu.pillbox", $.proxy(this.inputFocus, this));
    this.$element.on(
      "keydown.fu.pillbox",
      ".pillbox-add-item",
      $.proxy(this.inputEvent, this)
    );
    if (this.options.onKeyDown) {
      this.$element.on(
        "mousedown.fu.pillbox",
        ".suggest > li",
        $.proxy(this.suggestionClick, this)
      );
    }
  ...
  Pillbox.prototype = {
    ...
    removeItems: function (index, howMany) {
      ...
      if (!index) {
        ...
        for (count = 0; count < howMany; count++) {
          $currentItem = self.$pillGroup.find(
            "> .pill:nth-child(" + index + ")"
          );
    ...
    inputEvent: function (e) {
      ...
      if (this.acceptKeyCodes[e.keyCode]) {
        ...
        if (
          text.replace(/[ ]*\,[ ]*/, "").match(/\S/) ||
          (this.options.allowEmptyPills && text.length)
        ) {
          this._closeSuggestions();
          this.$addItem.hide();
      ...
      if (enable) {
        ...
        this.$pillGroup.find(".pill").each(function () {
          ...
          if (!full) {
            ...
            if (
              used +
                pill.outerWidth(true) +
                self.$addItemWrap.outerWidth(true) <=
              available
            ) {
    ...
    _openSuggestions: function (e, data) {
      ...
      if (data.data && data.data.length) {
        $.each(data.data, function (index, value) {
          ...
          var $suggestion = $(
            '<li data-value="' + val + '">' + value.text + "</li>"
          );
    ...
    _keySuggestions: function (e) {
      ...
      if (!$first.length) {
        ...
        if (!$next.length) {
          $next = dir
            ? this.$suggest.find("li:last")
            : this.$suggest.find("li:first");
        }
