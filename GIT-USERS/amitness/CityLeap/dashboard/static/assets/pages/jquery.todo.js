  ...
  (TodoApp.prototype.markTodo = function (todoId, complete) {
      ...
      for (var count = 0; count < this.$todoData.length; count++) {
        ...
        if (todoItem.done == true)
          this.$todoList.prepend(
            '<li class="list-group-item"><div class="checkbox checkbox-primary"><input class="todo-done" id="' +
              todoItem.id +
    ...
    (TodoApp.prototype.init = function () {
      ...
      $(document).on("change", this.$todoDonechk, function () {
        if (this.checked) $this.markTodo($(this).attr("id"), true);
        else $this.markTodo($(this).attr("id"), false);
        //regenerate list
        $this.generate();
      });
