...
var UIController = (function () {
    ...
    return {
        ...
        renderProfile: function(gitUser){
            document.querySelector(DOMstrings.userName).firstChild.innerHTML = "@" + gitUser.user.login;
            if(gitUser.user.name){
                document.querySelector(DOMstrings.firstName).firstChild.innerHTML = gitUser.user.name;
            } else {
                document.querySelector(DOMstrings.firstName).firstChild.innerHTML = 'Undefined';
            }
            if(gitUser.user.bio){
                document.querySelector(DOMstrings.bio).firstChild.innerHTML = gitUser.user.bio;
            }else {
                document.querySelector(DOMstrings.bio).firstChild.innerHTML = 'Undefined';
            }
            if(gitUser.user.avatar_url){
                document.querySelector(DOMstrings.userPic).children[0].src = gitUser.user.avatar_url;
            }
        },
        ...
        renderShadow: function(){
            ...
            document.querySelector(DOMstrings.mainContainer).style.borderRadius = "30px";
            document.querySelector(DOMstrings.searchBox).style.borderRadius = "30px";
            document.querySelector(DOMstrings.mainContainer).style.boxShadow = "10px 20px 15px #cac9c9";
            document.querySelector(DOMstrings.form).style.boxShadow = "10px 10px 5px #cac9c9";
        }
    }
