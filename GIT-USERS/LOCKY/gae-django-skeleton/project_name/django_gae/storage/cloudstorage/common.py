...
class GCSFileStat(object):
    ...
    def __cmp__(self, other):
        if not isinstance(other, self.__class__):
            raise ValueError(
                "Argument to cmp must have the same type. " "Expect %s, got %s",
...
def validate_bucket_name(name):
  ...
  """
    ...
    if not _GCS_BUCKET_REGEX.match(name):
        raise ValueError(
            "Bucket should be 3-63 characters long using only a-z,"
...
def validate_file_path(path):
  ...
  """
    ...
    if not _GCS_FULLPATH_REGEX.match(path):
        raise ValueError(
            "Path should have format /bucket/filename " "but got %s" % path
        )
...
def _process_path_prefix(path_prefix):
  ...
  """
    ...
    if not _GCS_PATH_PREFIX_REGEX.match(path_prefix):
        raise ValueError(
            "Path prefix should have format /bucket, /bucket/, "
...
def _validate_path(path):
  ...
  """
    ...
    if not isinstance(path, basestring):
        raise TypeError(
            "Path should be a string but is %s (%s)." % (path.__class__, path)
        )
  ...
  """
    ...
    for k, v in options.iteritems():
        ...
        if not isinstance(v, basestring):
            raise TypeError(
                "value %r for option %s should be of type basestring." % v, k
            )
