...
class TestUtils(TestCase):
    def test_collect_dependency_paths(self):
        with mock.patch("pkg_resources.get_distribution") as mock_get_d, mock.patch(
            "os.path.isdir"
        ) as mock_isdir, mock.patch("os.path.exists"):
    ...
    def test_collect_dependency_paths_fail(self):
        ...
        self.assertRaises(
            RequirementNotFoundError, collect_dependency_paths, "wrong/foo"
        )
    ...
    def test_make_symlinks_link_exists(self):
        with mock.patch("os.symlink"), mock.patch("os.path.islink"), mock.patch(
            "os.remove"
        ) as mock_remove:
