...
class TestCommands(TestCase):
    def tearDown(self):
        appengine_config = os.path.join(
            os.path.dirname(appengine_toolkit_settings.APP_YAML), "appengine_config.py"
        )
...
class TestCollectDeps(TestCommands):
    ...
    def test_call_package(self):
        with mock.patch(
            "appengine_toolkit.management.commands.collectdeps.collect_dependency_paths"
        ) as mock_collect:
    ...
    def test_call_req_file(self):
        with mock.patch("__builtin__.open"), mock.patch(
            "appengine_toolkit.management.commands.collectdeps.parse_requirements_file"
    ...
    def test_call_req_file_fail(self):
        with mock.patch("__builtin__.open"), mock.patch(
            ...
            self.assertRaises(
                ...
                call_command,
                "collectdeps",
                requirements_file="foofile",
                interactive=False,
            )

    def test_call_create_deps_dir(self):
        with mock.patch(
            "appengine_toolkit.management.commands.collectdeps.os.path.exists"
        ) as mock_exists, mock.patch(
            "appengine_toolkit.management.commands.collectdeps.os.mkdir"
