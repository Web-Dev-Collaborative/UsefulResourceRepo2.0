...
def process_literal(node, filtered_columns=[], negated=False):
    ...
    if negated and _op == "=":  # Explode
        return (
            ("OR", [("LIT", (column, ">", value)), ("LIT", (column, "<", value))]),
            filtered_columns,
        )
...
def parse_dnf(node, connection):
    ...
    if tree and len(tree[-1]) > 30:
        for and_branch in tree[-1]:
            ...
            for lit in (
                and_branch[-1] if and_branch[0] == "AND" else and_branch
            ):  # Go through each literal tuple
...
def parse_tree(node, connection, filtered_columns=None, negated=False):
    ...
    if node[0] in ["AND", "OR"]:
        new_children = []
        for child in node[1]:
            parsed_node, _columns = parse_tree(
                child, connection, filtered_columns, negated
            )
            if parsed_node:
    ...
    if node[0] == "LIT":
        parsed_lit, _columns = process_literal(
            node, filtered_columns=filtered_columns, negated=negated
        )
