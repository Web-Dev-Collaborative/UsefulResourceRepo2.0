...
def MainMenu():
    ...
    links = HTML.ElementFromURL(url).xpath(
        '//div[@id="rowDZ"]//div[contains(@class,"saveableWo")]/a/img/parent::a'
    )
    ...
    for link in links:
        ...
        dir.Append(
            Function(
                DirectoryItem(ImageViewer, title=title, thumb=thumb),
                title=title,
                url=url,
            )
        )
...
def ImageViewer(sender, title, url):
    dir = MediaContainer()
    photos = HTML.ElementFromURL(url).xpath(
        '//div[@id="galleryViewPhotos"]//div[@class="panel"]'
    )
    ...
    if len(photos):
        ...
        slideshow = HTML.ElementFromURL(url).xpath(
            '//div[@id="dzImagesPano"]//script[@type="text/javascript"]/text()'
        )[0]
