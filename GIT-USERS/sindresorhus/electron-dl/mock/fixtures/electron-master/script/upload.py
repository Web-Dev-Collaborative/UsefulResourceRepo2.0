...
from lib.config import PLATFORM, get_target_arch, get_env_var, s3_config, get_zip_name
...
def parse_args():
    ...
    parser.add_argument(
        "-v", "--version", help="Specify the version", default=ELECTRON_VERSION
    )
    parser.add_argument(
        "-p", "--publish-release", help="Publish the release", action="store_true"
    )
...
def release_electron_checksums(github, release):
    checksums = run_python_script("merge-electron-checksums.py", "-v", ELECTRON_VERSION)
    upload_io_to_github(
        github,
        release,
        "SHASUMS256.txt",
        StringIO(checksums.decode("utf-8")),
        "text/plain",
    )
...
def upload_sha256_checksum(version, file_path):
    ...
    s3put(
        ...
        "atom-shell/tmp/{0}".format(version),
        [checksum_path],
    )
