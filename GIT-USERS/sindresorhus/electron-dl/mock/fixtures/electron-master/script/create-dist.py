...
CHROMIUM_DIR = os.path.join(
    SOURCE_ROOT, "vendor", "download", "libchromiumcontent", "static_library"
)
...
TARGET_BINARIES_EXT = []
TARGET_DIRECTORIES = {
    "darwin": ["{0}.app".format(PRODUCT_NAME)],
    "win32": ["resources", "locales"],
    "linux": ["resources", "locales"],
}
...
def copy_ucrt_binaries():
    with _winreg.OpenKey(
        _winreg.HKEY_LOCAL_MACHINE, r"SOFTWARE\Microsoft\Windows Kits\Installed Roots"
    ) as key:
...
def copy_license():
    shutil.copy2(os.path.join(CHROMIUM_DIR, "..", "LICENSES.chromium.html"), DIST_DIR)
