...
HEADERS_FILES = ["common.gypi", "config.gypi"]
...
def main():
    ...
    node_headers_dir = os.path.join(args.directory, "node-{0}".format(args.version))
    iojs_headers_dir = os.path.join(args.directory, "iojs-{0}".format(args.version))
    iojs2_headers_dir = os.path.join(
        args.directory, "iojs-{0}-headers".format(args.version)
    )
...
def parse_args():
    ...
    parser.add_argument("-v", "--version", help="Specify the version", required=True)
    parser.add_argument(
        "-d",
        "--directory",
        ...
        for dirpath, _, filenames in os.walk(abs_path):
            for filename in filenames:
                ...
                if extension not in HEADERS_SUFFIX:
                    continue
                copy_source_file(
                    os.path.join(dirpath, filename), NODE_DIR, dist_headers_dir
                )
    for other_file in HEADERS_FILES:
        copy_source_file(os.path.join(NODE_DIR, other_file), NODE_DIR, dist_headers_dir)

    # Copy V8 headers from chromium's repository.
    src = os.path.join(SOURCE_ROOT, "vendor", "download", "libchromiumcontent", "src")
