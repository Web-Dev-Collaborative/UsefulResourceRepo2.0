...
IGNORE_FILES = [
    ...
    os.path.join("atom", "browser", "ui", "cocoa", "touch_bar_forward_declarations.h"),
    ...
    os.path.join("brightray", "browser", "mac", "bry_inspectable_web_contents_view.h"),
    os.path.join("brightray", "browser", "mac", "event_dispatching_window.h"),
    os.path.join("brightray", "browser", "mac", "notification_center_delegate.h"),
...
def main():
    ...
    atom_files = list_files(
        "atom", ["app", "browser", "common", "renderer", "utility"], ["*.cc", "*.h"]
    )
    call_cpplint(list(set(atom_files) - set(IGNORE_FILES)))

    brightray_files = list_files("brightray", ["browser", "common"], ["*.cc", "*.h"])
