...
def get_data_files_from_directory(
    ...
    np.random.shuffle(
        files
    )  # This avoids having large_file_0, large_file_1, ... subsequences
    return files
...
class Model(ABC):
    ...
    def get_default_hyperparameters(cls) -> Dict[str, Any]:
        return {
            ...
            "learning_rate": 0.01,
            "learning_rate_code_scale_factor": 1.0,
            "learning_rate_query_scale_factor": 1.0,
    ...
    def __init__(
        ...
        self.__code_encoders: OrderedDict[
            str, Any
        ] = OrderedDict()  # OrderedDict as we are using the order of languages a few times...
        ...
        self.__placeholders: Dict[
            str, Union[tf.placeholder, tf.placeholder_with_default]
        ] = {}
        ...
        if model_save_dir is None:
            self.__model_save_dir = os.environ.get(
                "PHILLY_MODEL_DIRECTORY", default="."
            )  # type: str
        else:
            self.__model_save_dir = model_save_dir  # type: str

        if log_save_dir is None:
            self.__log_save_dir = os.environ.get(
                "PHILLY_LOG_DIRECTORY", default="."
            )  # type: str
        else:
    ...
    def save(self, path: RichPath) -> None:
        variables_to_save = list(
            set(self.__sess.graph.get_collection(tf.GraphKeys.GLOBAL_VARIABLES))
        )
    ...
    def train_log(self, msg) -> None:
        log_path = os.path.join(self.__log_save_dir, f"{self.run_name}.train_log")
        with open(log_path, mode="a", encoding="utf-8") as f:
            f.write(msg + "\n")
        print(msg.encode("ascii", errors="replace").decode())

    def test_log(self, msg) -> None:
        log_path = os.path.join(self.__log_save_dir, f"{self.run_name}.test_log")
        with open(log_path, mode="a", encoding="utf-8") as f:
            f.write(msg + "\n")
        print(msg.encode("ascii", errors="replace").decode())
        ...
        with self.__sess.graph.as_default():
            ...
            if is_train:
                ...
                self.__summary_writer = tf.summary.FileWriter(
                    self.__tensorboard_dir, self.__sess.graph
                )
    ...
    def _make_model(self, is_train: bool) -> None:
        ...
        self.__placeholders["sample_loss_weights"] = tf.placeholder_with_default(
            input=np.ones(shape=[self.hyperparameters["batch_size"]], dtype=np.float32),
        ...
        with tf.variable_scope("code_encoder"):
            ...
            for (language, language_metadata) in sorted(
                self.__per_code_language_metadata.items(), key=lambda kv: kv[0]
            ):
                with tf.variable_scope(language):
                    self.__code_encoders[language] = self.__code_encoder_type(
                        label="code",
                        hyperparameters=self.hyperparameters,
                        metadata=language_metadata,
                    )
                    language_encoders.append(
                        self.__code_encoders[language].make_model(is_train=is_train)
                    )
            self.ops["code_representations"] = tf.concat(language_encoders, axis=0)
        with tf.variable_scope("query_encoder"):
            self.__query_encoder = self.__query_encoder_type(
                label="query",
                hyperparameters=self.hyperparameters,
                metadata=self.__query_metadata,
            )
            self.ops["query_representations"] = self.__query_encoder.make_model(
                is_train=is_train
            )

        code_representation_size = next(
            iter(self.__code_encoders.values())
        ).output_representation_size
        query_representation_size = self.__query_encoder.output_representation_size
        assert (
            code_representation_size == query_representation_size
    ...
    def _make_loss(self) -> None:
        if self.hyperparameters["loss"] == "softmax":
            ...
            per_sample_loss = tf.nn.sparse_softmax_cross_entropy_with_logits(
                labels=tf.range(
                    tf.shape(self.ops["code_representations"])[0]
                ),  # [0, 1, 2, 3, ..., n]
                logits=logits,
            )
        elif self.hyperparameters["loss"] == "cosine":
            query_norms = (
                tf.norm(self.ops["query_representations"], axis=-1, keep_dims=True)
                + 1e-10
            )
            code_norms = (
                tf.norm(self.ops["code_representations"], axis=-1, keep_dims=True)
                + 1e-10
            )
            cosine_similarities = tf.matmul(
                self.ops["query_representations"] / query_norms,
            ...
            neg_matrix = tf.diag(
                tf.fill(dims=[tf.shape(cosine_similarities)[0]], value=float("-inf"))
            )
            per_sample_loss = tf.maximum(
                0.0,
                self.hyperparameters["margin"]
            ...
            min_inf_matrix = tf.diag(
                tf.fill(dims=[tf.shape(logprobs)[0]], value=float("-inf"))
            )
            per_sample_loss = tf.maximum(
                0.0,
                self.hyperparameters["margin"]
            ...
            query_reps = tf.broadcast_to(
                query_reps,
                shape=[
                    tf.shape(query_reps)[0],
                    tf.shape(query_reps)[0],
                    tf.shape(query_reps)[1],
                ],
            )  # B*xBxD
            code_reps = tf.broadcast_to(
                code_reps,
                shape=[
                    tf.shape(code_reps)[0],
                    tf.shape(code_reps)[0],
                    tf.shape(code_reps)[1],
                ],
            )  # B*xBxD
            ...
            correct_distances = tf.expand_dims(
                tf.diag_part(all_pair_distances), axis=-1
            )  # Bx1

            pointwise_loss = tf.nn.relu(
                correct_distances - all_pair_distances + self.hyperparameters["margin"]
            )  # BxB
            pointwise_loss *= 1 - tf.eye(tf.shape(pointwise_loss)[0])

            per_sample_loss = tf.reduce_sum(pointwise_loss, axis=-1) / (
                tf.reduce_sum(
                    tf.cast(tf.greater(pointwise_loss, 0), dtype=tf.float32), axis=-1
                )
                + 1e-10
            )  # B
        else:
        ...
        self.ops["loss"] = tf.reduce_sum(per_sample_loss) / tf.reduce_sum(
    ...
    def _make_training_step(self) -> None:
        ...
        clipped_gradients, _ = tf.clip_by_global_norm(
            gradients, self.hyperparameters["gradient_clip"]
        )
        ...
        for (gradient, trainable_var) in zip(clipped_gradients, trainable_vars):
            ...
            if trainable_var.name.startswith("code_encoder/"):
                gradient *= tf.constant(
                    self.hyperparameters["learning_rate_code_scale_factor"],
                    dtype=tf.float32,
                )
            elif trainable_var.name.startswith("query_encoder/"):
                gradient *= tf.constant(
                    self.hyperparameters["learning_rate_query_scale_factor"],
                    dtype=tf.float32,
                )
    ...
    def load_metadata(
        ...
        def received_result_callback(
            metadata_parser_result: Tuple[Dict[str, Any], Dict[str, Dict[str, Any]]]
        ):
            (raw_query_metadata, per_code_language_metadata) = metadata_parser_result
            raw_query_metadata_list.append(raw_query_metadata)
            for (
                metadata_language,
                raw_code_language_metadata,
            ) in per_code_language_metadata.items():
                raw_code_language_metadata_lists[metadata_language].append(
                    raw_code_language_metadata
                )
        ...
        self.__query_metadata = self.__query_encoder_type.finalise_metadata(
            "query", self.hyperparameters, raw_query_metadata_list
        )
        for (
            language,
            raw_per_language_metadata,
        ) in raw_code_language_metadata_lists.items():
            self.__per_code_language_metadata[
                language
            ] = self.__code_encoder_type.finalise_metadata(
    ...
    def load_existing_metadata(self, metadata_path: RichPath):
        ...
        for hyper_name in hyper_names:
            ...
            if old_hyper_value != new_hyper_value:
                self.train_log(
                    "I: Hyperparameter %s now has value '%s' but was '%s' when tensorising data."
    ...
    def load_data_from_dirs(
        ...
        parallelize: bool = True,
    ) -> Union[LoadedSamples, Tuple[LoadedSamples, int]]:
    ...
    def load_data_from_files(
        ...
        if parallelize:
            ...
            per_file_results = [
                parse_data_file(*task_args) for task_args in tasks_as_args
            ]
    ...
    def __extend_minibatch_by_sample(
        ...
        if (
            is_train
            and sample[f"query_tokens_{QueryType.FUNCTION_NAME.value}"] is not None
            and random.uniform(0.0, 1.0)
            < self.hyperparameters["fraction_using_func_name"]
        ):
            query_type = QueryType.FUNCTION_NAME.value

        if include_query:
            batch_data["per_language_query_data"][language]["query_sample_ids"].append(
                sample_id
            )
        ...
        if include_code:
            batch_data["per_language_code_data"][language]["code_sample_ids"].append(
                sample_id
            )
            minibatch_is_full |= self.__code_encoders[
                language
        ...
        return (
            minibatch_is_full
            or batch_data["samples_in_batch"] >= self.hyperparameters["batch_size"]
        )

    def __minibatch_to_feed_dict(
        self,
        batch_data: Dict[str, Any],
        language_to_reweighting_factor: Optional[Dict[str, float]],
        is_train: bool,
    ) -> Tuple[Dict[tf.Tensor, Any], List[SampleId]]:
        """
        ...
        """
        final_minibatch = {
            self.__placeholders["dropout_keep_rate"]: self.hyperparameters[
                "dropout_keep_rate"
            ]
            if is_train
            else 1.0
        ...
        language_weights = []
        for (language, language_encoder) in self.__code_encoders.items():
            language_encoder.minibatch_to_feed_dict(
                batch_data["per_language_code_data"][language],
                final_minibatch,
                is_train,
            )
        ...
        self.__query_encoder.minibatch_to_feed_dict(
            full_query_batch_data, final_minibatch, is_train
        )
        if language_to_reweighting_factor is not None:
            final_minibatch[
        ...
        if (
            len(full_query_batch_data["query_sample_ids"]) > 0
        ):  # If we are only computing code representations, this will be empty
    ...
    def __split_data_into_minibatches(
        self,
        data: LoadedSamples,
        is_train: bool = False,
        include_query: bool = True,
        ...
        if compute_language_weightings:
            ...
            language_to_reweighting_factor = {
                language: float(total_num_samples)
                / (num_languages * num_samples_per_language)
                for (
                    language,
                    num_samples_per_language,
                ) in language_to_num_remaining_samples.items()
            }
        else:
        ...
        while len(language_to_num_remaining_samples) > 0:
            ...
            total_num_remaining_samples = sum(
                language_to_num_remaining_samples.values()
            )
            picked_language = np.random.choice(
                a=remaining_languages,
                p=[
                    float(language_to_num_remaining_samples[lang])
                    / total_num_remaining_samples
            ...
            )  # Note that indexing is 0-based and counting 1-based...
            language_to_num_remaining_samples[picked_language] -= 1
            if language_to_num_remaining_samples[picked_language] == 0:
                del language_to_num_remaining_samples[
                    picked_language
                ]  # We are done with picked_language now
            picked_sample = data[picked_language][
                language_to_idx_list[picked_language][picked_example_idx]
            ]

            # Add the example to the current minibatch under preparation:
            batch_data["samples_in_batch"] += 1
            batch_finished = self.__extend_minibatch_by_sample(
                batch_data,
                picked_sample,
            ...
            if batch_finished:
                feed_dict, original_sample_ids = self.__minibatch_to_feed_dict(
                    batch_data, language_to_reweighting_factor, is_train
                )
                yield feed_dict, batch_data[
                    "samples_in_batch"
                ], total_samples_used, original_sample_ids
                batch_data = self.__init_minibatch()
    ...
    def __run_epoch_in_batches(
        self, data: LoadedSamples, epoch_name: str, is_train: bool, quiet: bool = False
        ...
        data_generator = self.__split_data_into_minibatches(
            data, is_train=is_train, compute_language_weightings=True
        )
        samples_used_so_far = 0
        printed_one_line = False
        for (
            minibatch_counter,
            (batch_data_dict, samples_in_batch, samples_used_so_far, _),
        ) in enumerate(data_generator):
            if not quiet or (minibatch_counter % 100) == 99:
                print(
                    "%s: Batch %5i (has %i samples). Processed %i samples. Loss so far: %.4f.  MRR so far: %.4f "
                    % (
                        epoch_name,
                        minibatch_counter,
                        samples_in_batch,
                        samples_used_so_far - samples_in_batch,
                        loss,
                        mrr,
                    ),
                    flush=True,
            ...
            if (minibatch_counter % 100) == 0 and is_train:
                wandb.log(
                    {"train-loss": op_results["loss"], "train-mrr": op_results["mrr"]}
                )
        ...
        if printed_one_line:
            print("\r\x1b[K", end="")
        self.train_log(
            "  Epoch %s took %.2fs [processed %s samples/second]"
    ...
    def model_save_path(self) -> str:
        return os.path.join(self.__model_save_dir, f"{self.run_name}_model_best.pkl.gz")
    ...
    def train(
        ...
        with self.__sess.as_default():
            tf.set_random_seed(self.hyperparameters["seed"])
            train_data_per_lang_nums = {
                language: len(samples) for language, samples in train_data.items()
            }
            print(
                "Training on %s samples."
                % (
                    ", ".join(
                        "%i %s" % (num, lang)
                        for (lang, num) in train_data_per_lang_nums.items()
                    )
                )
            )
            valid_data_per_lang_nums = {
                language: len(samples) for language, samples in valid_data.items()
            }
            print(
                "Validating on %s samples."
                % (
                    ", ".join(
                        "%i %s" % (num, lang)
                        for (lang, num) in valid_data_per_lang_nums.items()
                    )
                )
            )
            ...
            if resume:
                ...
                best_val_mrr_loss, best_val_mrr, _ = self.__run_epoch_in_batches(
                    valid_data, "RESUME (valid)", is_train=False, quiet=quiet
                )
                self.train_log("Validation Loss on Resume: %.6f" % (best_val_mrr_loss,))
            else:
                init_op = tf.variables_initializer(
                    self.__sess.graph.get_collection(tf.GraphKeys.GLOBAL_VARIABLES)
                )
            ...
            epoch_number = 0
            while (
                epoch_number < self.hyperparameters["max_epochs"]
                ...
                train_loss, train_mrr, train_time = self.__run_epoch_in_batches(
                    train_data,
                ...
                val_loss, val_mrr, val_time = self.__run_epoch_in_batches(
                    valid_data,
                ...
                self.train_log(
                    " Validation:  Loss: %.6f | MRR: %.6f" % (val_loss, val_mrr)
                )
                ...
                for key in log:
                    if key != "epoch":
                        self._log_tensorboard_scalar(
                            tag=key, value=log[key], step=epoch_number
                        )
                ...
                if val_mrr > best_val_mrr:
                    ...
                    self.save(model_path)
                    self.train_log(
                        "  Best result so far -- saved model as '%s'." % (model_path,)
                    )
                else:
    ...
    def __compute_representations_batched(
        self,
        ...
        for raw_sample in raw_data:
            ...
            if valid_example:
                sample_to_tensorised_data_id.append(
                    (language, len(tensorized_data[language]))
                )
        ...
        for (
            ...
            (
                ...
                samples_in_batch,
                samples_used_so_far,
                batch_original_tensorised_data_ids,
            ),
        ) in enumerate(data_generator):
            op_results = self.__sess.run(
        ...
        tensorised_data_id_to_representation_idx = {
            tensorised_data_id: repr_idx
            for (repr_idx, tensorised_data_id) in enumerate(
                original_tensorised_data_ids
            )
        }
        ...
        for tensorised_data_id in sample_to_tensorised_data_id:
            if tensorised_data_id is None:
                ...
                reordered_representations.append(
                    computed_representations[
                        tensorised_data_id_to_representation_idx[tensorised_data_id]
                    ]
                )
        return reordered_representations

    def get_query_representations(
        self, query_data: List[Dict[str, Any]]
    ) -> List[Optional[np.ndarray]]:
        ...
        return self.__compute_representations_batched(
            query_data,
    ...
    def get_code_representations(
        self, code_data: List[Dict[str, Any]]
    ) -> List[Optional[np.ndarray]]:
