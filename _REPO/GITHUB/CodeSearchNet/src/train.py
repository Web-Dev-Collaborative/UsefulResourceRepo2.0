...
def run_train(
    ...
    train_data = model.load_data_from_dirs(
        train_data_dirs,
        is_test=False,
        max_files_per_dir=max_files_per_dir,
        parallelize=parallelize,
    )
    valid_data = model.load_data_from_dirs(
        valid_data_dirs,
        is_test=False,
        max_files_per_dir=max_files_per_dir,
        parallelize=parallelize,
...
def run(arguments, tag_in_vcs=False) -> None:
    ...
    train_data_dirs = test.expand_data_path(
        arguments["TRAIN_DATA_PATH"], azure_info_path
    )
    ...
    wandb.init(
        name=run_name,
        config={k: v for k, v in hyperparameters.items() if not isinstance(v, set)},
    )
    wandb.config.update(
        {
            "model-class": arguments["--model"],
    ...
    if testrun:
        ...
        compute_evaluation_metrics(
            model_path, arguments, azure_info_path, valid_data_dirs, test_data_dirs
        )
