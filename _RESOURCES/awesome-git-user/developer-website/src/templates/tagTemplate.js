                        ...
                        return (
                            <article className="article">
                                <Link to={post.node.fields.slug}>
                                    <h2 className="article-title">{post.node.frontmatter.title}</h2>
                                </Link>
                                ...
                                <div className="article-tags">
                                    {
                                        post.node.frontmatter.tags.map((tag, i) => {
                                            return <Link to={`/tags/${tag.toLowerCase()}/`}><small>{tag}</small></Link>
                                        })
                                    } 
                                </div>    
                            </article>
...
export const query = graphql`
    query tagTemplateQuery($tag: String) {
        posts: allMarkdownRemark(limit: 200 filter: {frontmatter: {tags: {in: [$tag]}}, fileAbsolutePath: {regex: "/post/"}} sort: { fields: [frontmatter___date], order: DESC}) {
            totalCount
            edges {
                node {
                    html 
