...
METADATA_READER_EXCEPTION_MATCH_TEXT = (
...
def run_freesound_extractor(audiofile):
    ...
    try:
        ...
        if MORE_THAN_2_CHANNELS_EXCEPTION_MATCH_TEXT in str(
            e
        ) or METADATA_READER_EXCEPTION_MATCH_TEXT in (str(e)):
...
def estimate_number_of_events(
    audiofile,
    audio,
    sample_rate=44100,
    region_energy_thr=0.5,
    silence_thr_scale=4.5,
    group_regions_ms=50,
):
    """
    ...
    def group_regions(regions, group_regions_ms):
        ...
        if len(regions) <= 1:
            grouped_regions = regions[
                :
            ]  # Don't do anything if only one region or no regions at all
        else:
            # Iterate over regions and mark which regions should be grouped with the following regions
            to_group = []
            for count, ((at0, at1, a_energy), (bt0, bt1, b_energy)) in enumerate(
                zip(regions[:-1], regions[1:])
            ):
    ...
    env_algo = essentia.standard.Envelope(attackTime=15, releaseTime=50)
    ...
    regions = [
        region for region in regions if region[2] > region_energy_thr
    ...
    if _is_single_event_cache is None:
        ...
        try:
            ...
            if MORE_THAN_2_CHANNELS_EXCEPTION_MATCH_TEXT in str(e):
                converted_audiofile = convert_to_wav(audiofile)
                audio_file = MonoLoader(
                    filename=converted_audiofile, sampleRate=sample_rate
                )
        ...
        if len(audio) / sample_rate > max_duration:
            ...
            _is_single_event_cache = (
                estimate_number_of_events(audiofile, audio, sample_rate=sample_rate)
                == 1
            )
...
def ac_rhythm_description(audiofile, fs_pool, ac_descriptors):
    ...
    if is_loop:
        ...
        tempo_confidence = (
            fs_pool["rhythm.bpm_confidence"] / 5.0
        )  # Normalize BPM confidence value to be in range [0, 1]
...
def ac_pitch_description(audiofile, fs_pool, ac_descriptors):
    ...
    ac_descriptors["note_confidence"] = float(
        fs_pool["lowlevel.pitch_instantaneous_confidence.median"]
    )
...
def ac_timbral_models(audiofile, ac_descriptors):
    ...
    try:
        timbre = timbral_models.timbral_extractor(
            converted_filename, clip_output=True, verbose=False
        )
        timbre["reverb"] = timbre["reverb"] == 1
        ac_descriptors.update(timbre)
    except Exception as e:
        logger.debug(
            '{0}: timbral models computation failed ("{1}")'.format(audiofile, e)
        )
...
def build_graph(ac_descriptors, uri=None):
    ...
    if ac_descriptors["lossless"]:
        g.add(
            (audioFile, NFO["compressionType"], Literal("nfo:losslessCompressionType"))
        )
    else:
...
def render_jsonld_output(g):
    ...
    frame = {
        "@type": str(AC["AudioFile"])
    }  # Apparently just by indicating the frame like this it already builds the desired output
    jsonld = g.serialize(
        format="json-ld", context=context
    ).decode()  # this gives us direct triple representation in a compact form
    jsonld = pyld.jsonld.frame(
        jsonld, frame, options={"documentLoader": dlfake}
    )  # this "frames" the JSON-LD doc but it also expands it (writes out full URIs)
    jsonld = pyld.jsonld.compact(
        jsonld, context, options={"documentLoader": dlfake}
    )  # so we need to compact it again (turn URIs into CURIEs)
    return jsonld
...
def analyze(
    ...
    if compute_timbral_models:
        MAX_SOUND_DURATION_FOR_TIMBRAL_MODELS = (
            30
        )  # Avoid computing timbral models for sound longer than 30 seconds to avoid too many slow computations
        if ac_descriptors["duration"] < MAX_SOUND_DURATION_FOR_TIMBRAL_MODELS:
            ac_timbral_models(audiofile, ac_descriptors)
        else:
            logger.debug(
                "{0}: skipping computation of timbral models as audio is longer than {1} seconds".format(
                    audiofile, MAX_SOUND_DURATION_FOR_TIMBRAL_MODELS
                )
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-v",
        "--verbose",
        help="if set, prints detailed info on screen during the analysis",
        action="store_const",
        const=True,
        default=False,
    )
    parser.add_argument(
        "-t",
        "--timbral-models",
        help="include descriptors computed from timbral models",
        action="store_const",
        const=True,
        default=False,
    )
    parser.add_argument(
        "-m",
        "--music-pieces",
        help="include descriptors designed for music pieces",
        action="store_const",
        const=True,
        default=False,
    )
    parser.add_argument(
        "-s",
        "--music-samples",
        help="include descriptors designed for music samples",
        action="store_const",
        const=True,
        default=False,
    )
    parser.add_argument(
        "-i",
        "--input",
        help="input audio file or input directory containing the audio files to analyze",
        required=True,
    )
    parser.add_argument(
        "-o",
        "--output",
        help="output analysis file or output directory where the analysis files will be saved",
        required=True,
    )
    parser.add_argument(
        "-f",
        "--format",
        help='format of the output analysis file ("json" or "jsonld", defaults to "jsonld")',
        default="jsonld",
    )
    parser.add_argument(
        "-u",
        "--uri",
        help='URI for the analyzed sound (only used if "jsonld" format is chosen)',
        default=None,
    )
    ...
    if os.path.isdir(args.input) and os.path.isdir(args.output):
        ...
        for input_file in input_files:
            output_file = os.path.join(
                args.output, "{}_analysis.json".format(input_file.stem)
            )
            analyze(
                str(input_file),
                output_file,
    ...
    # check if input argument points to a file
        ...
        raise ArgumentTypeError(
            "Make sure input and output arguments are both files or folders"
        )
