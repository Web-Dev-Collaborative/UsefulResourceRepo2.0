...
def regex_replace_all(args: Dict[str, Any], pattern: Pattern[str], repl: str) -> None:
  ...
  """
    ...
    contents_new = re.sub(pattern, rf"{repl}", contents, flags=re.MULTILINE | re.DOTALL)
    if contents_new != contents:
        fp.write_text(contents_new, encoding="utf-8")


def regex_between_groups_replace_all(
    args: Dict[str, Any], pattern: Pattern[str], repl: str
) -> None:
