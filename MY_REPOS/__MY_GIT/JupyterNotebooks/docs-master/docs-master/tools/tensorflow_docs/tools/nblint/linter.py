...
LintDict = Dict[
    decorator.Options.Scope, Dict[decorator.Options.Cond, List[decorator.Lint]]
]
...
class Linter:
  ...
  """
    ...
    def _run_lint(
        self, lint: decorator.Lint, lint_args: Dict[str, Any], status: "LinterStatus"
    ) -> "Linter.RunLintStatus":
    ...
    """
        ...
        return self.RunLintStatus(
            is_success, lint, lint_args, cond_fail_msg, fix_fn, fix_args
        )
    ...
    def run(
        self, path: pathlib.Path, lint_dict: LintDict, user_args_dict: Dict[str, Any]
    ) -> "LinterStatus":
    ...
    """
        ...
        for lint in lint_dict[decorator.Options.Scope.FILE][decorator.Options.Cond.ANY]:
            run_status = self._run_lint(lint, lint_args, status)
...
class LinterStatus:
  ...
  """
    ...
    def add_entry(self, lint, run_status, name=None, group=None, is_group_entry=False):
    ...
    def log_lint_message(
        self, msg: str, lint: decorator.Lint, verbose_msg: Optional[str] = None
    ) -> None:
