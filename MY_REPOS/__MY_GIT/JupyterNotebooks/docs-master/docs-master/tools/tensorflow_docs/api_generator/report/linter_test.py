...
class LinterTest(absltest.TestCase):
    def setUp(self):
        ...
        tree = {"TestClass": ["__init__", "method_one", "temp_c"]}
        reference_resolver = parser.ReferenceResolver.from_visitor(
            visitor=DummyVisitor(index=index, duplicate_of={}), py_module_names=["tf"]
        )
    ...
    def test_class_raises_lint(self):
        class_page_info = parser.docs_for_object(
            full_name="TestClass", py_object=TestClass, parser_config=self.parser_config
        )
        ...
        for test_report in test_api_report.api_report.symbol_metric:
            if (
                test_report.symbol_name == "TestClass"
                and test_report.object_type == api_report_pb2.ObjectType.CLASS
            ):
    ...
    def test_method_return_lint(self):
        ...
        for test_report in test_api_report.api_report.symbol_metric:
            if (
                test_report.symbol_name == "TestClass.method_one"
                and test_report.object_type == api_report_pb2.ObjectType.METHOD
            ):
