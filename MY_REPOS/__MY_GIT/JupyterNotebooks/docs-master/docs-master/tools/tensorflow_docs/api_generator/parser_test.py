...
class ParserTest(parameterized.TestCase):
    ...
    def test_docs_for_class(self):
        ...
        self.assertEqual(inspect.getdoc(TestClass).split("\n")[0], page_info.doc.brief)
        ...
        self.assertEqual(
            ["arg=&#x27;default&#x27;"], method_infos["a_method"].signature.arguments
        )
    ...
    def test_dataclass_attributes_table(self):
        ...
        index = {"ExampleDataclass": ExampleDataclass}
    ...
    def test_namedtuple_field_order(self):
        ...
        self.assertSequenceEqual(
            page_info._properties, sorted(page_info._properties, key=sort_key)
        )
    ...
    """
        ...
        index = {
            ...
            "ConcreteMutableMapping.__getitem__": ConcreteMutableMapping.__getitem__,
            "ConcreteMutableMapping.__setitem__": ConcreteMutableMapping.__setitem__,
        ...
        self.assertIn(
            ConcreteMutableMapping.get, [m.py_object for m in page_info.methods]
        )
    ...
    @parameterized.named_parameters(
        ...
        ("namedtuple", "ConcreteNamedTuple", "__new__", ConcreteNamedTuple.__new__),
        ("ClassUsingAttrs_eq", "ClassUsingAttrs", "__eq__", ClassUsingAttrs.__eq__),
        (
...
class TestReferenceResolver(absltest.TestCase):
    ...
    def testIsClasssAttr(self):
        result = parser.is_class_attr(
            "test_module.test_function", {"test_module": test_module}
        )
        self.assertFalse(result)

        result = parser.is_class_attr(
            "TestClass.test_function", {"TestClass": TestClass}
        )
...
class TestParseDocstring(absltest.TestCase):
    def test_split_title_blocks(self):
        ...
        returns = [item for item in docstring_parts if not isinstance(item, str)][-1]
...
class TestGenerateSignature(parameterized.TestCase, absltest.TestCase):
    def setUp(self):
        ...
        self.parser_config = parser.ParserConfig(
            ...
            index={},
            reverse_index={
                id(self.known_object): "location.of.object.in.api",
                id(
                    parser.extract_decorators
    ...
    @parameterized.named_parameters(
        (
            "deep_objects",
            Union[
                Dict[str, Dict[bool, parser.extract_decorators]],
                int,
                bool,
                parser.extract_decorators,
        ...
        (
            "callable_ellipsis_sig",
            Union[Callable[..., int], str],
            textwrap.dedent(
                """\
        Union[
            Callable[..., int],
        ...
        (
            "callable_args_sig",
            Union[Callable[[bool, parser.extract_decorators], float], int],
            textwrap.dedent(
                """\
        Union[
    ...
    def test_type_alias_signature(self, alias, expected_sig):
        info_obj = parser.TypeAliasPageInfo(
            full_name="tfdocs.api_generator.generate_lib.DocGenerator", py_object=alias
        )
