...
class ReferenceResolver(object):
    ...
    """
        ...
        filters = [
            IgnoreLineInBlock('<pre class="tfo-notebook-code-cell-output">', "</pre>"),
            IgnoreLineInBlock("```", "```"),
            IgnoreLineInBlock(
    ...
    def python_link(
        self, link_text, ref_full_name, relative_path_to_root, code_ref=True
    ):
    ...
    def _one_ref(self, match, relative_path_to_root, full_name=None):
        ...
        elif full_name is None or (
            "tf.compat.v" not in full_name and "tf.contrib" not in full_name
        ):
...
def _get_other_member_doc(
    obj: Any, parser_config: ParserConfig, extra_docs: Optional[Dict[int, str]]
) -> str:
...
def _parse_md_docstring(
  ...
  """
    ...
    raw_docstring = parser_config.reference_resolver.replace_references(
        raw_docstring, relative_path_to_root, full_name
    )
    ...
    def get_link(self, obj_full_name: str) -> str:
        relative_path_to_root = os.path.relpath(
            ...
            start=os.path.dirname(
                documentation_path(self._func_full_name, self._is_fragment)
            )
            or ".",
        )
    ...
    """
        ...
        for arg in args:
            ...
            if arg_name in self._type_annotations:
                typeanno = self.preprocess(
                    self._type_annotations[arg_name], arg.annotation
                )
    ...
    """
        ...
        for kwarg, ast_default in zip(kwargs, ast_defaults):
            ...
            if kname in self._type_annotations:
                typeanno = self.preprocess(
                    self._type_annotations[kname], kwarg.annotation
                )
...
class TypeAliasPageInfo(PageInfo):
  ...
  """
    ...
    def _link_type_args(
        self, obj: Any, reverse_index: Dict[int, str], linker: FormatArguments
    ) -> str:
        ...
        if getattr(obj, "__args__", None):
            ...
            origin_str = typing._type_repr(
                obj.__origin__
            )  # pylint: disable=protected-access # pytype: disable=module-attr
            result = self._custom_join(result, origin_str)
            return f"{origin_str}[{result}]"
        else:
            return typing._type_repr(
                obj
            )  # pylint: disable=protected-access # pytype: disable=module-attr
  ...
  """
    ...
    """
        ...
        for base in self.py_object.__mro__[1:]:
            ...
            base_doc = _parse_md_docstring(
                base, relative_path, self.full_name, parser_config, self._extra_docs
            )
    ...
    def _add_member(
        ...
        if obj_type is ObjType.PROPERTY:
            ...
            if issubclass(self.py_object, ProtoMessage):
                if member_info.short_name.endswith(
                    "_FIELD_NUMBER"
                ) or member_info.short_name in ["__slots__", "DESCRIPTOR"]:
                    return
    ...
    """
        ...
        for child_short_name in parser_config.tree[self.full_name]:
            ...
            if defining_class and defining_class.__name__ in [
                "CMessage",
                "Message",
                "MessageMeta",
            ]:
            ...
            child_doc = _parse_md_docstring(
                child, relative_path, self.full_name, parser_config, self._extra_docs
            )
    ...
    """
        ...
        for member_short_name in member_names:
            ...
            member_doc = _parse_md_docstring(
                member, relative_path, self.full_name, parser_config, self._extra_docs
            )
...
def docs_for_object(
  ...
  """
    ...
    page_info.set_doc(
        _parse_md_docstring(
            py_object, relative_path, full_name, parser_config, extra_docs
        )
    )
