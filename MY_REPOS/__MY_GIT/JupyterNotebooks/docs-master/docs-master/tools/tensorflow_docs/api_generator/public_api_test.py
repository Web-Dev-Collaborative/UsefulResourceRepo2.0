...
class PublicApiTest(absltest.TestCase):
    ...
    def test_private_map_child_removal(self):
        ...
        api_visitors = [
            public_api.PublicAPIFilter(base_dir="/", private_map={"tf.test": ["mock"]}),
            visitor,
        ]
    ...
    def test_local_definitions_filter(self):
        ...
        filtered_children = public_api.local_definitions_filter(
            ("tf", "keras", "layers"), tf.keras.layers, public_members(tf.keras.layers)
        )
    ...
    def test_explicit_package_contents_filter_removes_modules_imported_by_modules(self):
    ...
    def test_ignore_typing(self):
        ...
        children_after = public_api.ignore_typing("ignored", "ignored", children_before)
