...
EXPAND_TABLES = ["source_windows.md", "source.md"]
...
parser.add_argument(
    "--old_version", type=Version, required=True, help="The old version to replace"
)
parser.add_argument(
    "--new_version",
    type=Version,
    ...
    for ext in EXTS:
        for file_path in pathlib.Path(".").rglob("*" + ext):
            ...
            if file_path.name in EXPAND_TABLES:
                content = re.sub(
                    "(<tr>.*?){}(.*?</tr>)".format(re.escape(args.old_version.short())),
                    r"\g<1>{}\g<2>\n\g<0>".format(args.new_version.short()),
                    content,
                )
