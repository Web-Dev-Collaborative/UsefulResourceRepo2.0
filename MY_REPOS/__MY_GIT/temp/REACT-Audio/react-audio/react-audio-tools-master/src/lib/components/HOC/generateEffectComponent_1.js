...
const generateEffectComponent = (name, options, UIrenderer) => {
  ...
  class Effect extends Component {
    ...
    checkNextPropShape = (nextProps) => {
      if (nextProps.params) {
        ...
        if (uniques && uniques.length > 0) {
          throw new Error(
            `Check your prop updating code. You probably did not pass through all the expected default parameters for the ${name} effect`
          );
        }
      }
    };
    ...
    getComponentRenderer() {
      const { renderer, onChange, params, enabled, onEnableChange } =
        this.props;
