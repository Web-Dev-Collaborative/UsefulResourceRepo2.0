...
export class ComputerKeyboard extends Component {
  ...
  componentWillReceiveProps = (nextProps) => {
    ...
    if (nextProps.keyboardLayout !== keyboardLayout) {
      this.keySynth = generateKeyboardSynth(
        this.state.currentOctave,
        nextProps.keyboardLayout
      ); //init keysynth
    }
  ...
  shouldHandleKeyUpdate = (e) => {
    ...
    const keysToSearch =
      keyboardLayout === "azerty" ? azertyKeysToListenTo : qwertyKeysToListenTo;
  ...
  _handleKeyUp = (e) => {
    ...
    if (
      isOctaveDownKey(keyboardLayout, e.keyCode) ||
      e.keyCode === 88 ||
      e.keyCode === 16
    ) {
