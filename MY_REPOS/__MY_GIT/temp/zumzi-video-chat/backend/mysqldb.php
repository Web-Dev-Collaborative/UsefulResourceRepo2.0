<?php
include_once("mysql-wrapper.php");
...
function GetRowsFromDB($query, &$data, $callbackfn, $key='records', $keycount='reccount') {
	$result = NULL;
	//print $query;
	if($query != '')
		$result = QueryDB($query);

	//this is important so that console can make the array empty if no records found
	$data[$key] = array();

	if(NULL == $result) {
		if($keycount != '')
			$data[$keycount] = 0; // zero records
